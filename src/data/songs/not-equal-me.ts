import { Song } from './types';

export const notEqualMeSongs: Song[] = [
  {
    "id": "me-not-equal-me",
    "title": "≠ME",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2019-08-04",
      "centerMembers": [
        "冨田菜々風"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 5,
      "brightness": 5,
      "sadness": 1,
      "cuteness": 4,
      "power": 4,
      "calmness": 2,
      "romance": 4,
      "liveHype": 4,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "excited",
        "motivated"
      ],
      "situations": [
        "commute",
        "walking",
        "before_live"
      ],
      "weather": [
        "sunny"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "youth",
        "love",
        "dream"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "opening",
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "突き抜けるような疾走感と、初期衝動に満ちた眩しさが胸を打つ王道ポップス。グループの決意を刻んだ記念碑的なセルフタイトル曲です。",
      "recommendedFor": "新しい一歩を踏み出す勇気が欲しいときや、青春のまばゆいエネルギーを浴びたい人に。",
      "recommendedSituation": "爽やかな晴れた朝の通勤通学時や、大切な目標に向かう前の時間に。",
      "notRecommendedSituation": "深夜にベッドの中で、静かに物思いに沈みたいとき。",
      "recommendationText": "ノイミーの原点となる爽快感あふれるセルフタイトル曲です。突き抜ける疾走感と、純粋な決意を歌うまっすぐなボーカルが、あなたの中に眠る「好き」や夢に向かう情熱を力強く肯定してくれます。",
      "listeningSuggestion": "背筋を伸ばし、青空を見上げながら聴くのがおすすめです。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "motivated",
        "text": "自分の目標に向けてエンジンをかけたい今に最適な一曲です。メンバーたちのまっすぐな決意の歌声が、あなたの前進するエネルギーを呼び覚まします。"
      },
      {
        "conditionType": "timeOfDay",
        "condition": "morning",
        "text": "今日という新しい一日を、爽快なスタートで飾りたい朝にぴったりです。疾走感あふれるメロディが朝の眠気を吹き飛ばし、足取りを軽くしてくれます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "セルフタイトル曲としてグループの原点と決意を描いており、energyとbrightnessに最高の5を設定。初期曲の爽やかな夏らしさからseasonsにsummerを設定しました。",
      "confidence": 95
    }
  },
  {
    "id": "me-mahoroba-asterisk",
    "title": "まほろばアスタリスク",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2021-11-10",
      "centerMembers": [
        "冨田菜々風"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 4,
      "brightness": 4,
      "sadness": 3,
      "cuteness": 3,
      "power": 4,
      "calmness": 2,
      "romance": 5,
      "liveHype": 4,
      "nostalgia": 4,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "romantic",
        "sad",
        "nostalgic"
      ],
      "situations": [
        "driving",
        "relaxing",
        "before_live"
      ],
      "weather": [
        "cold",
        "snowy"
      ],
      "timeOfDay": [
        "evening",
        "night"
      ],
      "seasons": [
        "winter"
      ],
      "themes": [
        "unrequited_love",
        "youth",
        "memories"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "冬の夜空にきらめく星座のような、切なさと美しさが極限まで高まったエモーショナルポップス。ドラマチックな曲調と絶唱サビが胸を揺さぶります。",
      "recommendedFor": "届かない切ない片思いを抱えている人や、冬の夜空にきらめくエモーショナルな旋律に浸りたい人に。",
      "recommendedSituation": "冬の澄んだ星空の下を歩くときや、静かな夜のドライブの時間に。",
      "notRecommendedSituation": "夏の真昼のビーチで、ハイテンションにお祭り騒ぎをしたいとき。",
      "recommendationText": "冬のオリオン座の下で揺れる、届かない恋心を描いたノイミー屈指のエモーショナルポップです。ドラマチックなメロディと胸を締め付けるサビの絶唱が、切ない夜空の下のあなたを包み込むような美しい多幸感をもたらします。",
      "listeningSuggestion": "星がきれいに見える夜に、夜空を見上げながらヘッドホンでじっくり聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "sad",
        "text": "少し切ない恋心や叶わない思いに浸りたい夜に、この星のきらめきのような音楽を。サビのエモーショナルな歌声が、あなたの切ない涙を美しく肯定します。"
      },
      {
        "conditionType": "timeOfDay",
        "condition": "night",
        "text": "一日の喧騒が去った静かな夜に没入できるドラマチックな冬曲です。美しいストリングスとピアノの音が、静寂に満ちた夜の空間を特別な物語に変えてくれます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "歌詞に星空やオリオン座といった冬の天体に関するワードが複数存在しており、冬の澄んだ空気感を描写する楽曲コンセプトであるためwinterを設定。恋の切なさと感情の高まり（romance:5, emotionalIntensity:5）を主要な特徴としつつ、主用途（静かな夜や余韻）と矛盾しないようenergyとliveHypeを4に引き下げ相対化しました。",
      "confidence": 95
    }
  },
  {
    "id": "me-chocolate-melancholy",
    "title": "チョコレートメランコリー",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2022-02-16",
      "centerMembers": [
        "蟹沢萌子"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 4,
      "brightness": 2,
      "sadness": 2,
      "cuteness": 3,
      "power": 5,
      "calmness": 2,
      "romance": 4,
      "liveHype": 5,
      "nostalgia": 2,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "bittersweet",
        "lonely",
        "romantic",
        "excited"
      ],
      "situations": [
        "study",
        "work",
        "before_live"
      ],
      "weather": [
        "cold",
        "cloudy"
      ],
      "timeOfDay": [
        "night",
        "late_night"
      ],
      "seasons": [
        "winter"
      ],
      "themes": [
        "love"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "毒っけのある狂気的な愛を描いたゴシック調のラブソング。重厚なストリングスと鬼気迫る表現力が圧倒的な非日常を演出します。",
      "recommendedFor": "内に秘めた強い情念や執着を感じたいとき、妖艶でダークな美意識に没入したい人に。",
      "recommendedSituation": "少し寒さの残る曇りの日の夜や、何かに集中して作業している時に。",
      "notRecommendedSituation": "爽やかな朝一番のお出かけ前や、ピクニック中の明るいBGMとして。",
      "recommendationText": "重厚なストリングスとゴシックなメロディが絡み合う、狂気的な愛を表現したダークラブソングです。メンバーたちの鬼気迫る表現力と力強いビートが、あなたの日常に妖しい非日常の刺激をもたらし、退屈を吹き飛ばしてくれます。",
      "listeningSuggestion": "少し部屋の明かりを落とし、ヘッドホンでボーカルの細かなニュアンスを聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "excited",
        "text": "気持ちをディープに高揚させたい時に、このスリリングなゴシックロックを。重たいベースラインとエモーショナルなボーカルが、あなたの内に眠る熱量を呼び覚まします。"
      },
      {
        "conditionType": "situation",
        "condition": "work",
        "text": "一人の作業時間にダークで妖艶な世界観に没入したいとき。ドラマチックな曲展開が頭の回転を刺激し、周囲の雑音をかき消して集中力を引き出します。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "バレンタインデーに関連した冬のチョコレート作りやその贈り物としての情景、リリース時期（2月）からwinterを適用。甘みと苦みの両極端な愛憎・情念を表すためmoodsにbittersweetを追加。ゴシックロック調の激しさと狂愛のパワーをscoresに反映させ、夜の集中時間に合うバランスとしました。",
      "confidence": 90
    }
  },
  {
    "id": "me-hanakami-short",
    "title": "はにかみショート",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2022-11-23",
      "centerMembers": [
        "冨田菜々風"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 4,
      "brightness": 5,
      "sadness": 1,
      "cuteness": 5,
      "power": 4,
      "calmness": 2,
      "romance": 4,
      "liveHype": 4,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "cute",
        "happy",
        "romantic",
        "excited"
      ],
      "situations": [
        "commute",
        "walking",
        "driving"
      ],
      "weather": [
        "cold",
        "snowy"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "winter"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "opening",
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "髪を切った君への恋心を描く王道の冬ポップス。ホーンセクションの華やかな響きと圧倒的なかわいらしさが最高潮の多幸感を届けます。",
      "recommendedFor": "冬の凍える朝に心がぽかぽかと温まるハッピーを浴びたい人、ときめきを感じたい人に。",
      "recommendedSituation": "雪の舞う日の通学路や、冷え込んだ冬の日のドライブデートのBGMに。",
      "notRecommendedSituation": "静かに失恋の傷口を癒やしたい、暗い雨の降る真夜中。",
      "recommendationText": "髪をショートにした「君」との冬の恋を描いた、とびきり愛らしいウインターラブソングです。ホーンセクションがきらめく華やかなサビメロディが、冬の冷たい空気を一瞬で恋の甘さで満たしてくれ、今日一日をハッピーに始めるエネルギーを届けてくれます。",
      "listeningSuggestion": "白い息を吐きながら歩く通勤・通学路で、軽やかな足取りに合わせて聴くのが最高です。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "romantic",
        "text": "冬の澄んだ空気の中で、胸をときめかせる恋心を感じたい昼にぴったり。ポップできらめくアレンジが、あなたの心に温かい雪解けのような多幸感をもたらしてくれます。"
      },
      {
        "conditionType": "situation",
        "condition": "walking",
        "text": "冷たい風の中を元気に歩き出したいお散歩の時間に。明るく弾むようなポップサウンドが、冬のお出かけを彩り豊かな時間へと変えてくれます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "イルミネーションや白く凍える息、ゲレンデといった冬の風物詩的な歌詞フレーズの描写、およびリリース時期（11月）からwinterを設定。ポップできらめくかわいらしさを主用途（お出かけ前、昼）にマッチさせるため、moodsにcuteを追加しました。最大スコアの過度の集中を避けるため、brightnessとcutenessの2項目を最高値の5とし、energy、romance、liveHypeは4へ調整しました。",
      "confidence": 93
    }
  },
  {
    "id": "me-peonies",
    "title": "ピオニーズ",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2022-11-23",
      "centerMembers": [
        "鈴木瞳美",
        "谷崎早耶"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 2,
      "brightness": 2,
      "sadness": 3,
      "cuteness": 4,
      "power": 3,
      "calmness": 4,
      "romance": 4,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "healing",
        "calm",
        "romantic",
        "sad"
      ],
      "situations": [
        "relaxing",
        "before_sleep"
      ],
      "weather": [
        "cloudy",
        "rainy",
        "cold"
      ],
      "timeOfDay": [
        "night",
        "late_night"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "love",
        "unrequited_love"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "cool_down",
        "ending"
      ]
    },
    "recommendation": {
      "songImpression": "艶やかで気だるい耽美的なメロディが溶け合う極上のデュエット曲。静寂に包まれた夜のひとときに、二人だけの濃密な世界へ誘います。",
      "recommendedFor": "気だるく甘美な耽美的世界観に包まれたい夜や、静かな時間の中で耽溺したい人に。",
      "recommendedSituation": "雨の夜にしっとり部屋でアロマなどを焚きながら、目を閉じて過ごす時間に。",
      "notRecommendedSituation": "運動前のアップテンポでテンションを最高潮まで引き上げたい時。",
      "recommendationText": "鈴木瞳美と谷崎早耶の二人による、艶やかで耽美的な世界観を描いたユニット曲です。美しく憂いを帯びたメロディが、忙しい日常からあなたを切り離し、アンニュイで贅沢な夜の時間を提供します。",
      "listeningSuggestion": "お気に入りの香りを漂わせながら、静かな暗いお部屋でじっくりと浸るのがお勧めです。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "calm",
        "text": "一日の終わりに優しくクールダウンしたい夜にふさわしい、ミステリアスな一曲です。気だるく心地よいメロディが、あなたを穏やかなリラックス状態へと連れて行ってくれます。"
      },
      {
        "conditionType": "situation",
        "condition": "before_sleep",
        "text": "ベッドの中で目を閉じ、ゆっくり呼吸を整える眠りの導入に似合う甘美なバラード。美しいハーモニーが、あなたの意識をやさしく深い夢の世界へと手招きします。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "耽美的でアンニュイな雰囲気から、季節は特定の偏りを排してall_seasonsに変更。calmnessとnostalgiaを高めに調整し、眠り前やおやすみ前のBGMとして最適化しました。また、静かで穏やかなデュエットボーカルは聴き手をそっと包み込んで心を整える作用があるため、moodsにhealingを追加しました。",
      "confidence": 91
    }
  },
  {
    "id": "me-kimi-wa-kono-natsu-koi-wo-suru",
    "title": "君はこの夏、恋をする",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2021-07-14",
      "centerMembers": [
        "冨田菜々風"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 5,
      "brightness": 5,
      "sadness": 2,
      "cuteness": 4,
      "power": 4,
      "calmness": 2,
      "romance": 5,
      "liveHype": 5,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "excited",
        "romantic"
      ],
      "situations": [
        "driving",
        "before_live",
        "walking"
      ],
      "weather": [
        "sunny",
        "hot"
      ],
      "timeOfDay": [
        "daytime",
        "evening"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "opening",
        "build_up",
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "眩しい太陽と潮風を感じさせる疾走感あふれる夏ソング。王道のメロディラインと青春のときめきが胸いっぱいに広がります。",
      "recommendedFor": "まばゆい夏の太陽の下で、恋のときめきと青春の勢いを最高に楽しみたい人に。",
      "recommendedSituation": "夏の休日に海沿いをドライブするときや、大切な人に会う前の胸が高鳴る時間に。",
      "notRecommendedSituation": "静まり返った雪の冬の夜、静かに読書を楽しんでいるとき。",
      "recommendationText": "夏の高揚感と甘酸っぱい恋心を爽やかに疾走させた、ノイミーの夏を象徴する王道ソングです。爽快なサビのメロディがあなたの気分を夏色に染め上げてくれ、今日一日を輝かしい夏の青春物語へと変えてくれます。",
      "listeningSuggestion": "ドライブ中にボリュームを少し大きめにして、夏の風を浴びながら聴くのが最高です。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "excited",
        "text": "夏の日差しを浴びて、テンションを最大に解放したい昼休みにふさわしいナンバーです。キラキラしたサウンドが、あなたに熱いハッピーを届けます。"
      },
      {
        "conditionType": "situation",
        "condition": "driving",
        "text": "窓から入る夏の匂いを感じながら、目的地へ向かうドライブのお供に。駆け抜けるようなストリングスが、あなたのお出かけを爽やかに盛り上げてくれます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "曲名に『夏』を冠し、歌詞中にも海やひまわり、まばゆい太陽といった夏の鮮烈な風物詩が全編にわたって描写され、MV等のビジュアルイメージも夏をテーマにしているためsummerを設定。energyとbrightnessは最高の5を設定。疾走感のある明るい夏の青春恋愛気分に完全に寄り添う仕様です。",
      "confidence": 96
    }
  },
  {
    "id": "me-anti-confiture",
    "title": "アンチコンフィチュール",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2023-12-20",
      "centerMembers": [
        "冨田菜々風"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 4,
      "brightness": 2,
      "sadness": 1,
      "cuteness": 1,
      "power": 5,
      "calmness": 2,
      "romance": 1,
      "liveHype": 4,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "confident",
        "motivated"
      ],
      "situations": [
        "study",
        "work",
        "before_live"
      ],
      "weather": [
        "cloudy",
        "cold"
      ],
      "timeOfDay": [
        "night"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "self_acceptance",
        "challenge"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "『甘いだけが私じゃない』という強い自己肯定と闘志を刻んだ、硬質でクールなダンスナンバー。ソリッドな重低音と凛とした歌声が圧倒的な存在感を放ちます。",
      "recommendedFor": "他人の評価に左右されず、自分の価値観を強く信じて闘いたいと願う人に。",
      "recommendedSituation": "厳しい試験前、大事なプレゼン前など、自分を奮い立たせたい時間に。",
      "notRecommendedSituation": "ふわふわとした甘い恋に浸りたいときや、のんびりしたリラックスタイム。",
      "recommendationText": "「甘いだけの私じゃない」と力強く宣言する、ソリッドなエレクトロニック・ダンスナンバーです。強気でクールな歌声とエッジの効いたビートが、あなたの心に自立への意志と自信を呼び覚まし、障害に立ち向かう力を与えてくれます。",
      "listeningSuggestion": "イヤホンで大きめの音量で聴き、ビートの細かな刻みに没入するのがお勧めです。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "confident",
        "text": "他人の顔色を伺ってしまいそうなとき、自分自身の芯を強く保つための自己肯定曲です。力強いサビの歌詞が、あなたに凛とした強さをもたらします。"
      },
      {
        "conditionType": "situation",
        "condition": "study",
        "text": "高い目標に向けて頭をシャープに冴え渡らせたい作業時間にぴったり。重厚で緊迫感のあるサウンドが、あなたの集中力を一段上へ引き上げます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "自立や葛藤をテーマにしたダークで強気な世界観を表現するため、powerに最高の5を設定。季節は特定の情景の偏りがないためall_seasonsを設定しました。",
      "confidence": 93
    }
  },
  {
    "id": "me-last-chance-last-dance",
    "title": "ラストチャンス、ラストダンス",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2024-03-20",
      "centerMembers": [
        "冨田菜々風"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 4,
      "brightness": 4,
      "sadness": 3,
      "cuteness": 4,
      "power": 3,
      "calmness": 3,
      "romance": 4,
      "liveHype": 4,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "sad",
        "nostalgic",
        "motivated"
      ],
      "situations": [
        "commute",
        "walking",
        "relaxing"
      ],
      "weather": [
        "sunny",
        "cloudy"
      ],
      "timeOfDay": [
        "morning",
        "evening"
      ],
      "seasons": [
        "spring"
      ],
      "themes": [
        "unrequited_love",
        "youth",
        "memories"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "change",
        "cool_down"
      ]
    },
    "recommendation": {
      "songImpression": "桜の舞い散る季節を舞台にした、美しくも切ない青春卒業ポップス。ピアノとストリングスが織りなす疾走感の中に、淡い恋の終わりが描かれています。",
      "recommendedFor": "別れの季節に過去の思い出を愛おしく振り返りつつ、新しい春へ踏み出したい人に。",
      "recommendedSituation": "桜の花びらが舞う春の帰り道や、静かな日曜日の朝の時間に。",
      "notRecommendedSituation": "ライブ前の最高にテンションを高めて暴れたい瞬間。",
      "recommendationText": "春の切ない別れと片思いの終わりを描いた、爽やかで少しほろ苦い青春ポップスです。切なさを引きずりつつも前向きに進もうとする歌詞が、あなたの新しい始まりに寄り添い、優しいエールとなって響きます。",
      "listeningSuggestion": "春風を感じながら、通学路やよく歩いた街の景色の中で聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "nostalgic",
        "text": "過去の大切な記憶がよみがえり、少し甘酸っぱい感傷に包まれる夕暮れに最適です。疾走感のなかに宿る切なさが、あなたの思い出を優しく昇華させてくれます。"
      },
      {
        "conditionType": "situation",
        "condition": "commute",
        "text": "新しい環境へ向かう少し不安な朝の通勤時に寄り添います。前を向く強さを秘めたメロディが、あなたに今日の小さな勇気を注ぎ込んでくれます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "歌詞に『制服』『卒業』『桜』などの春の別れを象徴する単語が描かれているため、seasonsにはspringを明示。切なさと明るさが共存する楽曲として各種スコアを4前後に設定しました。",
      "confidence": 94
    }
  },
  {
    "id": "me-su-suki-janai",
    "title": "す、好きじゃない！",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2022-08-03",
      "centerMembers": [
        "冨田菜々風"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 4,
      "brightness": 5,
      "sadness": 1,
      "cuteness": 5,
      "power": 4,
      "calmness": 1,
      "romance": 4,
      "liveHype": 4,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "excited",
        "romantic",
        "playful"
      ],
      "situations": [
        "commute",
        "study",
        "before_live"
      ],
      "weather": [
        "sunny",
        "hot"
      ],
      "timeOfDay": [
        "daytime"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "opening",
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "全力で恋心を否定する『ツンデレ』な心情を、超高速のポップビートに乗せてコミカルに弾けさせた、至高の王道アイドルナンバー。",
      "recommendedFor": "素直になれないツンデレな恋心をハッピーに爆発させ、テンションを最大に上げたい人に。",
      "recommendedSituation": "天気の良い暑い日の昼休みや、退屈な授業や仕事の合間のリフレッシュに。",
      "notRecommendedSituation": "深夜に布団に入り、静かに瞑想して落ち着きたいとき。",
      "recommendationText": "超高速ビートの上を「好きじゃない！」と強がりな歌詞が駆け抜ける、最高にキュートなハイテンションポップです。底抜けに明るいパワーが、あなたの中にあった鬱屈を一気に吹き飛ばし、楽しいエネルギーを満タンにしてくれます。",
      "listeningSuggestion": "サビの早口フレーズに合わせて、リズムを取りながら楽しく聴くのが一番です。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "happy",
        "text": "文句なしに明るい一日を演出したい晴れた昼休みに。耳に飛び込む元気なフレーズが、あなたを一瞬で笑顔にしてくれます。"
      },
      {
        "conditionType": "situation",
        "condition": "before_live",
        "text": "ライブ前の待機時間に、コール＆レスポンスを想像して気分を最高潮にするのにぴったり。会場に向かうあなたの足どりを飛び跳ねるように軽くします。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "ひまわりや汗、夏休みといった単語が歌詞に散りばめられ、MVのロケーションも夏空であるためseasonsにはsummerを設定。明るさとかわいらしさを強調するためにbrightnessとcutenessを最高の5に設定しました。ツンデレで素直になれないハッピー＆コミカルな心情を表現した遊び心溢れる楽曲であるため、moodsにplayfulを追加しました。",
      "confidence": 95
    }
  },
  {
    "id": "me-omowaseburikko",
    "title": "想わせぶりっこ",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2023-09-06",
      "centerMembers": [
        "鈴木瞳美"
      ],
      "verificationStatus": "verified"
    },
    "scores": {
      "energy": 4,
      "brightness": 4,
      "sadness": 1,
      "cuteness": 5,
      "power": 3,
      "calmness": 2,
      "romance": 5,
      "liveHype": 4,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "excited",
        "romantic"
      ],
      "situations": [
        "walking",
        "relaxing",
        "before_live"
      ],
      "weather": [
        "sunny"
      ],
      "timeOfDay": [
        "daytime",
        "evening"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "love"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "opening",
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "『ちゅーする？』などの刺激的なセリフが散りばめられた、あざと可愛さ全振りの超王道アイドルポップス。甘い多幸感で聴き手を包み込みます。",
      "recommendedFor": "鈴木瞳美のセンター曲がもたらす、究極の「あざとかわいい」甘い世界に飛び込んで癒やされたい人に。",
      "recommendedSituation": "お気に入りのカフェでくつろいでいる時間や、お出かけ中の楽しい時間に。",
      "notRecommendedSituation": "人生のシリアスな悩みに向き合って、深く熟考している真面目な時間。",
      "recommendationText": "王道の甘さと遊び心が満載の、最高にガーリーな恋バナポップです。あざとかわいい台詞や掛け合いが、あなたの心の中に可愛いお祭り騒ぎを起こし、聴くだけで気分を最高に甘く引き立ててくれます。",
      "listeningSuggestion": "お気に入りのコスメやおめかしアイテムを眺めながら聴くと、より世界観に没入できます。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "romantic",
        "text": "大好きな人や推しのことを考えて胸がいっぱいな時に。この上なくあま〜いメロディが、あなたの甘酸っぱい妄想を可愛くバックアップします。"
      },
      {
        "conditionType": "situation",
        "condition": "relaxing",
        "text": "頭を使う作業はお休みにして、とにかく楽しい気持ちで満たされたい休憩時間に。ふわふわと軽い恋のフレーズが、あなたを優しくハッピーに包み込みます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "あざとく可愛い恋愛ソングとしての魅力を極限まで引き立たせるため、cutenessとromanceを最高の5に設定。季節は特定の偏りがないためall_seasonsを設定しました。",
      "confidence": 94
    }
  },
  {
    "id": "me-miracle-road",
    "title": "ミラクル！",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2021-11-10",
      "centerMembers": [],
      "verificationStatus": "needs-review"
    },
    "scores": {
      "energy": 5,
      "brightness": 5,
      "sadness": 1,
      "cuteness": 3,
      "power": 5,
      "calmness": 2,
      "romance": 1,
      "liveHype": 4,
      "nostalgia": 1,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "motivated",
        "confident"
      ],
      "situations": [
        "commute",
        "study",
        "work"
      ],
      "weather": [
        "sunny",
        "cloudy"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "friendship",
        "challenge",
        "dream"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "opening",
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "泥臭くも前向きに限界へ挑み続ける姿を、ブラスサウンドを交えた熱い疾走ビートで応援するスポ根系青春アンセム。",
      "recommendedFor": "部活動や自分の限界に挑戦している人、前向きに泥臭く頑張りたい全ての人へ。",
      "recommendedSituation": "絶対に勝ちたい試験の日や、今日の仕事のスイッチを入れたい朝一番に。",
      "notRecommendedSituation": "失恋に沈んでしまい、そっとしておいてほしい静かな夕暮れ。",
      "recommendationText": "青春の熱さと夢に向かう努力を歌い上げる、まばゆいスポ根系青春アンセムです。明るいホーンセクションの伴奏が、あなたに今日のやる気をもたらし、心の中に強い火を灯してくれます。",
      "listeningSuggestion": "大きく一歩を踏み出し、リズムに合わせて行進するようなイメージで聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "motivated",
        "text": "目標を掴み取るために、もう一歩努力したいあなたの背中をグッと押してくれるエールソングです。メンバーたちの元気な歌声が、あなたの情熱を後押しします。"
      },
      {
        "conditionType": "timeOfDay",
        "condition": "morning",
        "text": "憂鬱な朝のスタートを、奇跡を信じる前向きなパワーで塗り替えたい時に。明るい朝の光を浴びながら聴くことで、モチベーションが一気に活性化します。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "部活動や夢に向かって限界まで努力する人を真っ直ぐに鼓舞する力強い楽曲。アップテンポでエネルギッシュな明るさを表現するためenergyやpowerを高く設定。ノスタルジー要素は極めて低いためnostalgiaは1、全力の情熱を反映してemotionalIntensityは4としました。季節の明確な限定はないためall_seasonsを維持しつつ人間レビューでの確認を要します。",
      "confidence": 92
    }
  },
  {
    "id": "me-rainbow-melody",
    "title": "虹が架かる瞬間",
    "group": "not-equal-me",
    "factualData": {
      "releaseDate": "2021-04-07",
      "centerMembers": [],
      "verificationStatus": "needs-review"
    },
    "scores": {
      "energy": 3,
      "brightness": 4,
      "sadness": 2,
      "cuteness": 3,
      "power": 3,
      "calmness": 4,
      "romance": 2,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 3
    },
    "tags": {
      "moods": [
        "calm",
        "healing",
        "nostalgic"
      ],
      "situations": [
        "relaxing",
        "before_sleep",
        "walking"
      ],
      "weather": [
        "sunny",
        "cloudy",
        "rainy"
      ],
      "timeOfDay": [
        "morning",
        "evening"
      ],
      "seasons": [
        "spring",
        "all_seasons"
      ],
      "themes": [
        "friendship",
        "gratitude",
        "memories"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "cool_down",
        "ending"
      ]
    },
    "recommendation": {
      "songImpression": "試練を共に乗り越えたメンバー同士の絆と感謝を、雨上がりの青空へ架かる虹になぞらえて温かく紡ぐミディアムナンバー。",
      "recommendedFor": "困難を乗り越えて仲間との絆に感謝したい時や、やさしい光に包まれて心を癒やしたい人に。",
      "recommendedSituation": "雨上がりの夕方にゆっくり空を眺めるときや、おやすみ前の静かな時間に。",
      "notRecommendedSituation": "ここからモチベーションを高めて、アップテンポで大暴れしたい瞬間。",
      "recommendationText": "つらい雨の時期を乗り越え、空に虹が架かる瞬間を共に迎えるメンバーの絆を描いた温かいミディアムポップです。優しいアコースティックなサウンドが、あなたの心をやさしく撫でてくれ、張り詰めていた緊張をほぐします。",
      "listeningSuggestion": "ゆっくりとお茶を飲みながら、またはベッドの中で体力を休めている時に聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "healing",
        "text": "少し心が疲れてしまった時に、静かにあなたを癒やしてくれる温かい処方箋のような一曲です。メンバーがそっと語りかけるような歌声が、あなたの強張った心をゆっくりと解きほぐします。"
      },
      {
        "conditionType": "timeOfDay",
        "condition": "evening",
        "text": "一日の仕事を終えた帰り道の夕景にそっと馴染む、お疲れ様のメロディです。空の色の移り変わりを見つめながら、穏やかな時の流れを体感できます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "つらい時期を共に乗り越えてきた仲間への感謝と歩みを振り返る温かなミディアムテンポのポップス。懐かしさや優しさを重視し、nostalgiaを4、心の強さを込めてemotionalIntensityを3に設定。季節タグには春が含まれていますが、歌詞全体の絆に焦点を当てており、詳細な季節の根拠は人間レビューが必要です。",
      "confidence": 90
    }
  },
  {
    "id": "me-kimi-no-oto-datta-nda",
    "title": "君の音だったんだ",
    "group": "not-equal-me",
    "category": "group",
    "factualData": {
      "releaseDate": "2019-10-30",
      "releaseTitle": "ズルいよ ズルいね",
      "participatingMembers": [
        "needs-review"
      ],
      "verificationStatus": "partially-verified",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://equal-love.jp/discography/detail/6/",
          "note": "Official Discography Title Details"
        },
        {
          "field": "release",
          "url": "https://equal-love.jp/discography/detail/6/",
          "note": "Official Discography release date"
        }
      ]
    },
    "scores": {
      "energy": 4,
      "brightness": 4,
      "sadness": 2,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 3
    },
    "tags": {
      "moods": [
        "happy"
      ],
      "situations": [
        "commute",
        "walking"
      ],
      "weather": [
        "sunny"
      ],
      "timeOfDay": [
        "daytime"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "メンバーたちの瑞々しくのびのびとした歌声と心地よいアレンジが魅力的な、爽やかで王道アイドルらしさが溢れる楽曲です。",
      "recommendedFor": "心地よいリズムに乗せて、軽快なポップスを味わいたい人に。",
      "recommendedSituation": "徒歩での移動中や、晴れた日の散歩のお供に。",
      "notRecommendedSituation": "激しいビートでテンションを極限まで高めたいとき。",
      "recommendationText": "日常の通勤・通学時間や、静かにリラックスしたいときに、耳元を心地よく彩ってくれます。気持ちを少しだけ明るい方向へ向けたいときや、前向きに一日をスタートしたいときにおすすめです。",
      "listeningSuggestion": "散歩をしながら、周囲の風景に溶け込ませて聴くのがおすすめです。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "happy",
        "text": "明るく前向きなメロディが、あなたの何気ない一日にそっと彩りを添えてくれます。"
      },
      {
        "conditionType": "situation",
        "condition": "commute",
        "text": "毎日の通勤や通学の電車の中で、心地よくリズムに浸りたいときに向いています。"
      }
    ],
    "analysisBasis": {
      "interpretationNotes": "公式リリース情報に基づく全員曲。バランスのとれたポップ調アレンジ。",
      "confidence": 85
    },
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "recordType": "song"
  },
  {
    "id": "me-ai-kudasaimase",
    "title": "愛くださいませ",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2026-06-24",
      "releaseTitle": "「愛くださいませ/ここでファーストキッス」＜ノイミー盤＞",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/78/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/76/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/77/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/75/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "cute",
        "playful"
      ],
      "situations": [
        "getting_ready",
        "driving"
      ],
      "weather": [
        "sunny",
        "cloudy"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "love",
        "longing"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "「愛をください」という一途でキュートなおねだり感情を、アップテンポでエネルギッシュなメロディに乗せて甘酸っぱく届けるポップナンバーです。",
      "recommendedFor": "チャーミングでエネルギーに溢れるアイドルポップスが好きで、前向きで少し切ない乙女心に共感したいリスナーに適しています。",
      "recommendedSituation": "休日の朝の身支度中や、ドライブのスタート時に流して気分を盛り上げたい時におすすめです。",
      "notRecommendedSituation": "静かに心を落ち着かせたい読書中や、就寝前のリラックスタイムには刺激が強く不向きです。",
      "recommendationText": "キュートさと強い願いが交錯する歌詞世界がポップなビートに乗せて爽快に弾け、聴く人に元気と一途なときめきを分け与えてくれます。",
      "listeningSuggestion": "メロディの疾走感とボーカルの甘く力強い響きを存分に楽しめるよう、ヘッドホンでお出かけ中に聴くのが最適です。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "cute",
        "text": "一途で甘酸っぱい乙女心の切実なおねだりを、キュートで弾けるボーカルが全開で応援してくれます。"
      },
      {
        "conditionType": "situation",
        "condition": "driving",
        "text": "爽快なアップテンポのサウンドトラックとして、車内の雰囲気を一気に華やかに彩ります。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、強い恋愛感情と甘えが共存するキュートポップ。おねだりの切実さから emotionalIntensity は4とし、現在志向の可愛らしさであるため nostalgia は2と評価。既存の明るいトーン（energy:3, brightness:3）や恋のテーマとの大きな矛盾は見られない。既存の季節タグは維持するが、既存タグや季節タグの整合性の確認は人間レビューが必要。",
      "confidence": 80
    },
    "releases": [
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜ノイミー盤＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/78/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type B＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/76/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type C＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/77/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type A＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/75/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/78/",
        "https://not-equal-me.jp/discography/detail/76/",
        "https://not-equal-me.jp/discography/detail/77/",
        "https://not-equal-me.jp/discography/detail/75/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-koko-de-first-kiss",
    "title": "ここでファーストキッス",
    "group": "not-equal-me",
    "category": "group",
    "factualData": {
      "releaseDate": "2026-06-24",
      "releaseTitle": "愛くださいませ/ここでファーストキッス",
      "verificationStatus": "verified",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/78/",
          "note": "Official Reaudit Verified Evidence Source"
        }
      ],
      "centerMembers": [],
      "participatingMembers": [],
      "lyricist": "指原莉乃",
      "composer": "本多友紀",
      "arranger": "脇眞富"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "excited",
        "playful"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "evening"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "初々しい恋の初期衝動や、ファーストキッスへの憧れと駆け引きを描いた作品です。",
      "recommendedFor": "ときめく恋心を描いた楽曲を好む方や、明るい気持ちで音楽を楽しみたい方に適しています。",
      "recommendedSituation": "夕方の通学・通勤時間や、お出かけの準備をしながら軽やかな気分になりたい時に向いています。",
      "notRecommendedSituation": "静かな深夜にじっくりと心を落ち着かせたい時や、深くリラックスしたい就寝前のBGMにはテンポが速すぎるため不向きです。",
      "recommendationText": "初々しい恋の初期衝動や、ファーストキッスへの憧れと駆け引きを描いた作品です。甘酸っぱい恋の始まりに想いを馳せたいときや、心を弾ませたいひとときに合います。",
      "listeningSuggestion": "お出かけ前の準備時間や散歩の合間に、軽快なテンポに合わせて聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "playful",
        "text": "少しいたずらっぽく、恋を楽しむような気分になりたい瞬間に、弾けるメロディがそっと寄り添います。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "お出かけの準備をする時間に流すことで、曲が持つ明るいサウンドが軽やかなステップを後押しするでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、恋のときめきを描いたポップなアッパーチューン。初々しい恋心を表現するためemotionalIntensityを4、回想的な要素からnostalgiaを3に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 81
    },
    "releases": [
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜ノイミー盤＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/78/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type B＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/76/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type C＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/77/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type A＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/75/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/78/",
        "https://not-equal-me.jp/discography/detail/76/",
        "https://not-equal-me.jp/discography/detail/77/",
        "https://not-equal-me.jp/discography/detail/75/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-summerhaze",
    "title": "Summer haze",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2026-06-24",
      "releaseTitle": "「愛くださいませ/ここでファーストキッス」＜ノイミー盤＞",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/78/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/76/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/77/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/75/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "bittersweet",
        "dreamy"
      ],
      "situations": [
        "walking",
        "alone_time"
      ],
      "weather": [],
      "timeOfDay": [
        "evening"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "youth",
        "longing"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "cool_down"
      ]
    },
    "recommendation": {
      "songImpression": "夏の夕暮れの淡い空気感を漂わせる、切なくノスタルジックなメロディが特徴的なポップスと受け取れます。",
      "recommendedFor": "夏の思い出やセンチメンタルな余韻に浸りたい方、穏やかで情感豊かな音楽を求める方に適しています。",
      "recommendedSituation": "夏の日の夕方の散歩中や、静かな部屋で一人きりで過ごす時間に向いています。",
      "notRecommendedSituation": "テンションを上げて走り出したい朝の準備時間や、ライブ前のような興奮を高めたい場面には落ち着きすぎているため適していません。",
      "recommendationText": "温かみと切なさを帯びた歌声が耳に優しく残り、夏の終わりを惜しむような情景を想起させます。",
      "listeningSuggestion": "夕暮れ時に、ヘッドホンでボーカルの細かな息遣いや微細な強弱に集中して聴くのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "timeOfDay",
        "condition": "evening",
        "text": "西日が傾く時間帯に聴くことで、曲が描く夕暮れ時の淡い寂しさと愛おしさがより身近に感じられるでしょう。"
      },
      {
        "conditionType": "situation",
        "condition": "alone_time",
        "text": "誰にも邪魔されない一人の時間に流すことで、ノスタルジックな夏の空気感に深く浸ることができます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、夏の夕景を想起させるノスタルジックなミディアムナンバー。センチメンタルな情感を反映してemotionalIntensityを4、夏の終わりの回想感からnostalgiaを4に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜ノイミー盤＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/78/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type B＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/76/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type C＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/77/"
      },
      {
        "title": "「愛くださいませ/ここでファーストキッス」＜CD+DVD/Type A＞",
        "releaseType": "single",
        "releaseDate": "2026-06-24",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/75/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/78/",
        "https://not-equal-me.jp/discography/detail/76/",
        "https://not-equal-me.jp/discography/detail/77/",
        "https://not-equal-me.jp/discography/detail/75/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-haitateki-fighter",
    "title": "排他的ファイター",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2025-12-10",
      "releaseTitle": "「排他的ファイター」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/69/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "excited",
        "rebellious"
      ],
      "situations": [
        "getting_ready",
        "walking"
      ],
      "weather": [],
      "timeOfDay": [
        "night"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "challenge",
        "identity"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "強い意志と自立心を表現するような、エッジの効いたダンスビートとアグレッシブな雰囲気が特徴的な楽曲と解釈されます。",
      "recommendedFor": "困難に立ち向かう強いエネルギーをもらいたい方や、アッパーなサウンドでモチベーションを高めたい方に適しています。",
      "recommendedSituation": "新しい挑戦に向けて気持ちを奮い立たせたい場面や、アクティブに体を動かす時間に向いています。",
      "notRecommendedSituation": "心を静めて穏やかな眠りにつきたい夜の時間や、ゆったりとした静粛を求める状況でのBGMには適していません。",
      "recommendationText": "力強いリズムセクションと凛としたボーカルワークが、聴く人の内なる気概を刺激し、前に進む原動力を後押しします。",
      "listeningSuggestion": "音量を少し大きめに設定し、イヤホンやスピーカーの重低音に耳を傾けながらリズムを体全体で捉えるように聴くのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "rebellious",
        "text": "ブレない強さや反発するような強い意志を感じたい時に聴くことで、曲が放つ強烈なサウンドが心強い味方になります。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "大切なイベントや勝負事の前に聴くことで、モチベーションを高め、気持ちをしっかりと引き締める準備が整うでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、葛藤や決意を力強く歌い上げるダンスナンバー。強い意志を表現するためemotionalIntensityを5、現在進行形の気概を描くためnostalgiaを2に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 84
    },
    "releases": [
      {
        "title": "「排他的ファイター」",
        "releaseType": "single",
        "releaseDate": "2025-12-10",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/69/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/69/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-kyunkawa-jinsei",
    "title": "きゅんかわ人生",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2025-12-10",
      "releaseTitle": "排他的ファイター",
      "centerMembers": [
        "櫻井もも"
      ],
      "participatingMembers": [],
      "lyricist": "指原莉乃",
      "composer": "Ryo Shiraki / 新Q",
      "arranger": "新Q",
      "verificationStatus": "verified",
      "verifiedSources": [
        {
          "field": "youtube",
          "url": "https://www.youtube.com/watch?v=L8AQC1Gk4-M",
          "note": "Official Reaudit Verified Evidence Source"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 3
    },
    "tags": {
      "moods": [
        "happy",
        "cute"
      ],
      "situations": [
        "getting_ready",
        "walking"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "self_acceptance",
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "自分らしさを肯定する前向きなメッセージを、ポップで可愛らしいメロディに乗せて歌うキュートな楽曲と受け取れます。",
      "recommendedFor": "自己肯定感を高めたい時や、日常の小さな幸せに目を向けたい方、愛らしい雰囲気に包まれたい方に適しています。",
      "recommendedSituation": "朝のメイク時間やお出かけの準備中、よく晴れた休日の散歩のお供に向いています。",
      "notRecommendedSituation": "深い哀愁に浸りたい一人きりの夜や、静粛で集中力を要する作業中のBGMとしては世界観が合わず不向きです。",
      "recommendationText": "ありのままの自分を愛することをテーマにした、自己肯定のメッセージを持つ作品です。\n自分らしさを大切にしたい日や、気持ちを軽くしたい時間に合います。",
      "listeningSuggestion": "帰り道や放課後に、ありのままの自分を大切にする気持ちで聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "cute",
        "text": "ありのままの自分を愛することをテーマにした、自己肯定のメッセージを持つ作品です。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "お出かけ前のメイクやコーディネートを整える時間に流すことで、ハッピーな気分で一日を始める準備が整うでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、自己肯定と可愛らしさを描いたポップソング。明るい日常を描くためnostalgiaを3、心地よく調和した情緒からemotionalIntensityを3に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「排他的ファイター」",
        "releaseType": "single",
        "releaseDate": "2025-12-10",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/69/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/69/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-hanbunko-christmas",
    "title": "はんぶんこクリスマス",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2025-12-10",
      "releaseTitle": "「排他的ファイター」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/69/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "romantic",
        "cute"
      ],
      "situations": [
        "relaxing",
        "alone_time"
      ],
      "weather": [],
      "timeOfDay": [
        "night"
      ],
      "seasons": [
        "winter"
      ],
      "themes": [
        "love",
        "memories"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "冬のイルミネーションや冷たい空気を想起させる、温かでロマンチックなメロディが特徴的なウィンターポップスと受け取れます。",
      "recommendedFor": "クリスマスシーズンの温かい恋愛模様を味わいたい方や、甘く少し切ないメロディに包まれたい方に適しています。",
      "recommendedSituation": "冬の夜にお部屋で温かい飲み物を飲みながら過ごす時間や、静かな夜のひとときにBGMとして流すのに向いています。",
      "notRecommendedSituation": "夏の猛暑日のアクティブな運動中や、朝の爽快なスタートダッシュを飾りたい場面には季節感やテンポが合わず適していません。",
      "recommendationText": "優しく包み込むような歌声と冬らしいストリングスの響きが、心温まるような二人だけの特別な時間を演出します。",
      "listeningSuggestion": "静かな夜の部屋で、間接照明を灯しながら、スピーカーから穏やかに流して楽しむのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "season",
        "condition": "winter",
        "text": "寒さが厳しくなる季節に聴くことで、曲が描く冬の恋の温もりや愛おしさがじんわりと心に広がります。"
      },
      {
        "conditionType": "timeOfDay",
        "condition": "night",
        "text": "一日の終わりに近づく夜の静けさの中で流すことで、ロマンチックなメロディが穏やかな時間に調和するでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、冬の恋愛の温もりを描いたロマンチックなポップソング。愛おしさを表現するためemotionalIntensityを4、回想的な甘酸っぱさからnostalgiaを4に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「排他的ファイター」",
        "releaseType": "single",
        "releaseDate": "2025-12-10",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/69/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/69/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-mobunoderella",
    "title": "モブノデレラ",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2025-04-30",
      "releaseTitle": "「モブノデレラ/神様の言うとーり！」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/61/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "sad",
        "lonely"
      ],
      "situations": [
        "alone_time",
        "train_ride"
      ],
      "weather": [],
      "timeOfDay": [
        "night",
        "late_night"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "unrequited_love",
        "longing"
      ],
      "tempos": [
        "slow"
      ],
      "playlistRoles": [
        "cool_down"
      ]
    },
    "recommendation": {
      "songImpression": "恋愛における切ない片思いや孤独感を、繊細で静かなピアノの調べに乗せて歌うバラードと受け取れます。",
      "recommendedFor": "報われない恋の切なさにじっくりと浸りたい方や、静かな音楽に心を寄せたい方に適しています。",
      "recommendedSituation": "深夜に一人きりの部屋で過ごす時間や、帰りの電車の窓から夜景を眺めて物思いに耽る時に向いています。",
      "notRecommendedSituation": "朝の爽やかな目覚めの瞬間や、元気を出して一日をスタートさせたいアクティブな場面のBGMには適していません。",
      "recommendationText": "繊細で透明感のある歌声が胸に深く染み渡り、隠された恋心の寂しさをそっと包み込みます。",
      "listeningSuggestion": "部屋の明かりを暗くし、ヘッドホンを使用してボーカルの細かなニュアンスや余韻までじっくり聴くのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "timeOfDay",
        "condition": "late_night",
        "text": "深夜の静寂の中で聴くことで、曲が持つ孤独な美しさと切ない恋心の描写がより静かに心に響くでしょう。"
      },
      {
        "conditionType": "situation",
        "condition": "alone_time",
        "text": "一人きりで考え事をしたい時間に流すことで、静かな旋律が胸の奥にある素直な感情に寄り添います。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、片思いの寂しさを描いた繊細なバラード。切ない恋心を表現するためemotionalIntensityを4、ノスタルジックな感傷からnostalgiaを4に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「モブノデレラ/神様の言うとーり！」",
        "releaseType": "single",
        "releaseDate": "2025-04-30",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/61/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/61/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-kamisama-no-iutoori",
    "title": "神様の言うとーり！",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2025-04-30",
      "releaseTitle": "「モブノデレラ/神様の言うとーり！」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/61/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 3
    },
    "tags": {
      "moods": [
        "happy",
        "playful"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "spring"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "恋する乙女の祈るような願望を、弾けるポップスサウンドで前向きに歌うハッピーな楽曲と受け取れます。",
      "recommendedFor": "キュートな恋愛ソングを聴いて明るい気分になりたい方や、前向きなメッセージに背中を押されたい方に適しています。",
      "recommendedSituation": "朝の身支度を整える時間や、明るい日差しの差し込む時間帯の通学・通勤路でのリスニングに向いています。",
      "notRecommendedSituation": "厳かで静粛な雰囲気を求める作業時間や、一人で深く考え事をしたい静かな夜には元気すぎるため不向きです。",
      "recommendationText": "ポップなビートと愛らしさを重視した歌声が心地よく調和し、日常に軽快なリズムをプラスします。",
      "listeningSuggestion": "よく晴れた日の朝に、お気に入りのヘッドホンを装着し、リズムに合わせて歩きながら聴くのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "playful",
        "text": "恋の楽しさを素直に喜び、ウキウキした感情をシェアしたい瞬間に、曲の弾けるメロディが華を添えます。"
      },
      {
        "conditionType": "timeOfDay",
        "condition": "morning",
        "text": "一日の始まりである朝の時間に聴くことで、曲が放つ明るいエネルギーがすっきりとした目覚めをサポートするでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、恋の願いを可愛らしく歌うハッピーなポップソング。明るい前向きさを表現するためemotionalIntensityを3、爽快な日常感からnostalgiaを3に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「モブノデレラ/神様の言うとーり！」",
        "releaseType": "single",
        "releaseDate": "2025-04-30",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/61/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/61/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-cafe-rakuen",
    "title": "カフェ樂園",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2025-04-30",
      "releaseTitle": "モブノデレラ/神様の言うとーり！",
      "centerMembers": [
        "永田詩央里"
      ],
      "participatingMembers": [
        "永田詩央里",
        "蟹沢萌子",
        "冨田菜々風"
      ],
      "lyricist": "指原莉乃",
      "composer": "古川貴浩",
      "arranger": "古川貴浩",
      "verificationStatus": "verified",
      "verifiedSources": [
        {
          "field": "youtube",
          "url": "https://www.youtube.com/watch?v=vA5O8Pq9j44",
          "note": "Official Reaudit Verified Evidence Source"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 3
    },
    "tags": {
      "moods": [
        "healing",
        "cute"
      ],
      "situations": [
        "relaxing",
        "alone_time"
      ],
      "weather": [],
      "timeOfDay": [
        "daytime"
      ],
      "seasons": [
        "spring"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "お気に入りのカフェで過ごすような心地よい時間を想起させる、日常の小さな幸せと可愛らしさを描いた穏やかなポップスと受け取れます。",
      "recommendedFor": "忙しい日常から離れて心を落ち着かせたい方や、穏やかで可愛らしいメロディを好む方に適しています。",
      "recommendedSituation": "休日の昼下がりに温かい飲み物を片手にお部屋で過ごす時間や、のんびりと読書をする時のBGMに向いています。",
      "notRecommendedSituation": "深夜の張り詰めた緊張感の中で集中したい時や、激しいロックサウンドを求めてテンションを高めたい時には不向きです。",
      "recommendationText": "お気に入りのカフェで過ごすような、穏やかな日常の幸せを描いた作品です。\n休憩時間や、少し落ち着きたい午後に合います。",
      "listeningSuggestion": "休日の午後や休憩時間に、お気に入りのカフェで過ごす時間を思い浮かべながら聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "situation",
        "condition": "relaxing",
        "text": "お気に入りのカフェで過ごすリラックスタイムと、穏やかな日常の幸せを描いた作品です。"
      },
      {
        "conditionType": "timeOfDay",
        "condition": "daytime",
        "text": "日差しの差し込む昼下がりの時間に流すことで、曲が持つ穏やかな木漏れ日のような雰囲気が日常を彩ります。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、カフェのような心地よい空間と日常の愛おしさを描いた温かみのある曲。穏やかな安らぎを重視し、nostalgiaを3、緩やかで優しい情緒からemotionalIntensityを3に設定。既存の穏やかなスコア（calmness:3, cuteness:3）との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「モブノデレラ/神様の言うとーり！」",
        "releaseType": "single",
        "releaseDate": "2025-04-30",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/61/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/61/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-natsu-ga-kitakara",
    "title": "夏が来たから",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2024-08-28",
      "releaseTitle": "「夏が来たから」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/51/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "excited"
      ],
      "situations": [
        "walking",
        "driving"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "youth",
        "love"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "夏のまばゆい光や高鳴る期待感を想起させる、爽快で勢いのあるポップチューンと受け取れます。",
      "recommendedFor": "夏の青空の下で気分を盛り上げたい方や、爽やかでエネルギッシュなサマーソングを聴きたい方に適しています。",
      "recommendedSituation": "天気の良い夏の日のドライブや、晴れた日の散歩、活動的な屋外イベントへの道中に向いています。",
      "notRecommendedSituation": "雨の降る静かな梅雨の午後や、落ち着いた眠りを迎えたい就寝前の時間には賑やかすぎるため適していません。",
      "recommendationText": "爽やかなボーカルと弾けるリズムが調和し、夏特有の開放的でハッピーなムードを演出します。",
      "listeningSuggestion": "晴れ渡る夏の日の屋外で、ポータブルスピーカーやイヤホンを通じて、風を感じながら軽やかに聴くのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "season",
        "condition": "summer",
        "text": "夏の太陽が照りつける時間帯に聴くことで、曲が描く青空の開放感がより鮮明に心に響くでしょう。"
      },
      {
        "conditionType": "situation",
        "condition": "driving",
        "text": "晴れた日のドライブシーンで窓を開け、風を感じながら流すことで、爽快な疾走感が高まるでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、夏の到来を喜ぶ爽快なポップナンバー。輝かしいエネルギーを表現するためemotionalIntensityを4、爽やかな夏のノスタルジーからnostalgiaを3に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「夏が来たから」",
        "releaseType": "single",
        "releaseDate": "2024-08-28",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/51/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/51/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-gomen-majide",
    "title": "ごめん、マジで",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2024-08-28",
      "releaseTitle": "「夏が来たから」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/51/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "playful",
        "excited"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "daytime",
        "evening"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "youth",
        "friendship"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "コミカルで親しみやすい日常のやり取りを、軽快なポップサウンドとユーモラスな調子で描いたポップチューンと受け取れます。",
      "recommendedFor": "肩の力を抜いてクスッと笑えるような楽しい音楽を聴きたい方や、気楽なテンションで気分転換をしたい方に適しています。",
      "recommendedSituation": "友人との待ち合わせへの移動中や、何気ない日常の移動時間、肩の力を抜いて準備をしたい時に向いています。",
      "notRecommendedSituation": "深い失恋の痛みに寄り添ってほしい静かな夜や、厳かな空気を保ちたいシリアスな場面には世界観が合わず適していません。",
      "recommendationText": "遊び心のある台詞調のパートと親しみやすいメロディが調和し、気取らない楽しげな雰囲気を演出します。",
      "listeningSuggestion": "移動中に周囲の安全に配慮しながら、気楽な気分で口ずさむように聴くのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "playful",
        "text": "何気ない日常のハプニングを笑い飛ばし、陽気な気分を味わいたい瞬間に、この軽快なメロディがフィットします。"
      },
      {
        "conditionType": "situation",
        "condition": "commute",
        "text": "通学や通勤などの移動時間に流すことで、ユーモラスな世界観が日常の移動を楽しく彩ってくれるでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、日常の他愛ないやり取りをコミカルに描くポップナンバー。会話調の掛け合いの楽しさを表現するためemotionalIntensityを4、軽妙で現実味のある性質からnostalgiaを2に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「夏が来たから」",
        "releaseType": "single",
        "releaseDate": "2024-08-28",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/51/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/51/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-saikyo-no-lovesong",
    "title": "最強のラブソング",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2024-08-28",
      "releaseTitle": "「夏が来たから」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/51/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "cute"
      ],
      "situations": [
        "getting_ready",
        "walking"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "spring"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "ひたむきで愛らしい恋愛感情を、優しく爽やかなメロディに乗せてストレートに届けるポップソングと受け取れます。",
      "recommendedFor": "純粋なラブソングで温かい気持ちになりたい方や、可愛らしく王道なアイドルポップスを好む方に適しています。",
      "recommendedSituation": "晴れた日の穏やかな散歩中や、お出かけ前のハッピーな気分を盛り上げたい時間帯に向いています。",
      "notRecommendedSituation": "厳かで静粛な作業中のバックグラウンドや、重厚でシリアスな感情に寄り添う音楽を必要とする場面には甘すぎるため適していません。",
      "recommendationText": "温かみのあるメロディラインと愛らしさに満ちたボーカルが調和し、日常に恋心の華やぎをもたらします。",
      "listeningSuggestion": "晴れ渡る青空の下で、ヘッドホンを通して優しい歌声に耳を傾けながら、のんびりと聴くのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "cute",
        "text": "とことん素直で可愛い恋心に寄り添いたい瞬間に聴くことで、曲の明るい旋律がハッピーな感情を後押しします。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "お出かけの前に身支度をしながら流すことで、これから始まる一日が楽しみになるような明るい雰囲気を演出できるでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、ストレートな愛おしさを描いた可愛らしいポップソング。ひたむきな恋愛感情を表現するためemotionalIntensityを4、爽やかで少し回想的な愛らしさからnostalgiaを3に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「夏が来たから」",
        "releaseType": "single",
        "releaseDate": "2024-08-28",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/51/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/51/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-22",
    "title": "デートの後、22時",
    "group": "not-equal-me",
    "category": "group",
    "factualData": {
      "releaseDate": "2023-09-06",
      "releaseTitle": "「想わせぶりっこ」",
      "verificationStatus": "verified",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/40/",
          "note": "Official Reaudit Verified Evidence Source"
        }
      ],
      "centerMembers": [],
      "participatingMembers": [],
      "lyricist": "指原莉乃",
      "composer": "木下めろん",
      "arranger": "木下めろん"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "bittersweet",
        "cute"
      ],
      "situations": [
        "commute",
        "alone_time"
      ],
      "weather": [],
      "timeOfDay": [
        "night"
      ],
      "seasons": [
        "autumn"
      ],
      "themes": [
        "love",
        "longing"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "デート直後の愛おしさと寂しさ、夜22時の別れ際の複雑な恋心を描いた作品です。",
      "recommendedFor": "好きな人との楽しい時間の後の寂しさに寄り添ってほしい方や、甘く切ない雰囲気の恋愛曲を好む方に適しています。",
      "recommendedSituation": "デートの帰り道の電車内や、自宅の部屋で一日の出来事を振り返りながら過ごす夜に向いています。",
      "notRecommendedSituation": "早朝の爽やかな目覚めの瞬間や、大人数が集まって盛り上がるようなパーティーシーンのBGMには情緒が合わず適していません。",
      "recommendationText": "デート直後の愛おしさと寂しさ、そして夜22時の別れ際を描いた作品です。楽しかった時間の余韻に浸りたい帰り道や、大切な人を想う静かな夜に合います。",
      "listeningSuggestion": "帰りの電車の窓から夜景を眺めながら、ゆっくりと曲の世界観に浸って聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "timeOfDay",
        "condition": "night",
        "text": "一日の終わりに近づく夜の静けさの中で、デートの記憶を振り返りながら聴くことで、胸の切なさにそっと寄り添います。"
      },
      {
        "conditionType": "situation",
        "condition": "commute",
        "text": "デート終わりの帰り道、一人きりの移動時間に聴くことで、曲が描く恋心の余韻を感じられるでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、デート後の寂しさと愛おしさを描いた甘く切ない恋愛ソング。帰り道のノスタルジックな感傷を反映してnostalgiaを4、恋心の高まりからemotionalIntensityを4に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「想わせぶりっこ」",
        "releaseType": "single",
        "releaseDate": "2023-09-06",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/40/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/40/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    id: 'me-hatsukoi-comeback',
    title: '初恋カムバック',
    group: 'not-equal-me',
    category: 'unknown',
    factualData: {
      releaseDate: '2023-12-20',
      releaseTitle: '「アンチコンフィチュール」',
      centerMembers: ["谷崎早耶"],
      singingMembers: [],
      lyricist: '指原莉乃',
      composer: 'HaTo',
      arranger: 'めんま',
      verificationStatus: 'verified',
      verifiedSources: [
        { field: 'release', url: 'https://not-equal-me.jp/discography/detail/44/' }
      ]
    },
    scores: {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    tags: {
      "moods": [
            "cute",
            "bittersweet"
      ],
      "situations": [
            "getting_ready",
            "commute"
      ],
      "weather": [],
      "timeOfDay": [
            "morning",
            "daytime"
      ],
      "seasons": [
            "spring"
      ],
      "themes": [
            "unrequited_love",
            "memories"
      ],
      "tempos": [
            "medium"
      ],
      "playlistRoles": [
            "build_up"
      ]
    },
    recommendation: {
      songImpression: 'かつての恋心の再燃と甘酸っぱい記憶を、可愛らしくもどこか切ないメロディに乗せて歌うミディアムナンバーと受け取れます。',
      recommendedFor: '過去の淡い恋の思い出に優しく浸りたい方や、温かみのあるメロディに寄り添ってほしい方に適しています。',
      recommendedSituation: '朝の身支度中や、お出かけ時の穏やかな移動時間、静かな昼下がりのBGMに向いています。',
      notRecommendedSituation: '深夜に静かに心を無にして眠りたい時や、ハードなロックサウンドを求めてテンションを高めたい時には適していません。',
      recommendationText: '昔のピュアな恋心を思い出すような、甘酸っぱく爽快で明るいメロディが心地よい王道の青春アイドルラブソングです。',
      listeningSuggestion: '天気の良い日の散歩や、初恋のような甘酸っぱいトキメキを感じて明るい気分になりたい時におすすめです。',
      otherDisplayText: 'ピュアな恋心を歌う、明るく爽快な王道青春ラブソング。'
    },
    recommendationVariants: [
      {
            "conditionType": "mood",
            "condition": "cute",
            "text": "とことん素直な甘酸っぱさに包まれたい時、優しく可愛らしいボーカルがその感情にそっと寄り添います。"
      },
      {
            "conditionType": "situation",
            "condition": "getting_ready",
            "text": "お出かけ前のメイク時間や服を選ぶ時間に聴くことで、一日を少し特別な気分でスタートできるでしょう。"
      }
],
    enabled: true,
    recordType: 'song',
    needsReview: true,
    reviewRequirement: 'needs_review'
  },
  {
    "id": "me-heroine-to-ookami",
    "title": "ヒロインとオオカミ",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2023-12-20",
      "releaseTitle": "「アンチコンフィチュール」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/44/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "playful"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "daytime"
      ],
      "seasons": [
        "winter"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "ファンタジックで遊び心あふれる世界観を、賑やかでキャッチーなメロディに乗せて軽快に歌うポップチューンと受け取れます。",
      "recommendedFor": "ファンタジー感のある可愛らしい音楽を楽しみたい方や、気分の上がる曲で日常を楽しく彩りたい方に適しています。",
      "recommendedSituation": "朝の通学・通勤時間や、お出かけの準備中にウキウキした気分を高めたい時に向いています。",
      "notRecommendedSituation": "厳粛な会議前の集中したい時間や、穏やかな気持ちでリラックスしたい就寝前の静寂な夜には賑やかすぎるため適していません。",
      "recommendationText": "表情豊かなボーカルワークとポップなビートが調和し、物語の登場人物になったような楽しい空間を演出します。",
      "listeningSuggestion": "移動中に、軽快なテンポに合わせて歩きながら、イヤホンで賑やかな音の重なりを楽しむのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "playful",
        "text": "ちょっとした物語の主人公になったような遊び心のある気分を楽しみたい時、ポップな旋律がその感情を彩ります。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "お出かけ前のメイク時間や服を選ぶ時間に聴くことで、一日を楽しい気分でスタートできるでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、ファンタジックな恋のかけ引きを描くアップテンポなポップソング。楽しげな活力を表現するためemotionalIntensityを4、現在進行形のファンタジー感からnostalgiaを2に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「アンチコンフィチュール」",
        "releaseType": "single",
        "releaseDate": "2023-12-20",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/44/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/44/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-tenshi-wa-dokohe",
    "title": "天使は何処へ",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2023-04-12",
      "releaseTitle": "「天使は何処へ」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/26/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 4,
      "brightness": 2,
      "sadness": 2,
      "cuteness": 1,
      "power": 5,
      "calmness": 1,
      "romance": 1,
      "liveHype": 4,
      "nostalgia": 2,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "excited",
        "confident",
        "rebellious"
      ],
      "situations": [
        "study",
        "work",
        "before_live"
      ],
      "weather": [
        "cloudy",
        "rainy"
      ],
      "timeOfDay": [
        "evening",
        "night"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "self_acceptance",
        "challenge",
        "independence"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "重厚でエッジの効いたダンスビートに乗せ、自己の存在価値や葛藤を激しく突きつける、圧倒的で強いメッセージソング。",
      "recommendedFor": "自分を取り巻く限界を打ち破り、強い意志で困難に立ち向かうエネルギーをチャージしたい人に。",
      "recommendedSituation": "高い目標に挑戦する直前の集中力を高めたいときや、夜のハードなトレーニングに。",
      "notRecommendedSituation": "休日のお昼時に、のんびりと優しい日差しを浴びながらリラックスして過ごしたいとき。",
      "recommendationText": "これまでのイメージを覆す、ソリッドで力強いシンセサウンドとラップパートが融合した挑戦的ダンスポップです。爆発的な感情表現があなたの殻を破り、心の中に眠る不屈の闘志を目覚めさせます。",
      "listeningSuggestion": "ステレオの音量を少し上げ、低音のビートとタイトなリズム感を全身で感じながら聴いてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "confident",
        "text": "自己主張の強さと不屈の意志を奮い立たせる、最高にハードなダンスナンバー。ブレない自信を持って前に進みたい瞬間をサポートします。"
      },
      {
        "conditionType": "situation",
        "condition": "before_live",
        "text": "ライブ前に会場の熱気と同化し、全身のテンションを極限まで引き上げるのに最適。エッジの効いたリズムが鼓動を加速させます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "releases": [
      {
        "title": "「天使は何処へ」",
        "releaseType": "single",
        "releaseDate": "2023-04-12",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/26/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/26/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "葛藤を抱えながらもブレない自己の強さを表現した、エッジの効いたダンスナンバー。圧倒的な迫力と力強さを表現するためpowerを5、感情の激しさからemotionalIntensityを5に設定しました。ノスタルジーはほとんどないためnostalgiaは2。明確な季節のモチーフはなく、意志の強さを描いたテーマであるため、all_seasonsを設定。根拠は人間レビューでの再検証を仰ぎます。",
      "confidence": 95
    }
  },
  {
    "id": "me-2",
    "title": "2時半ろけんろー",
    "group": "not-equal-me",
    "category": "group",
    "factualData": {
      "releaseDate": "2023-04-12",
      "releaseTitle": "「天使は何処へ」",
      "verificationStatus": "verified",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/26/",
          "note": "Official Reaudit Verified Evidence Source"
        }
      ],
      "centerMembers": [],
      "participatingMembers": [],
      "lyricist": "指原莉乃",
      "composer": "鈴木エレカ / JOE",
      "arranger": "JOE"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "excited",
        "playful"
      ],
      "situations": [
        "getting_ready",
        "driving"
      ],
      "weather": [],
      "timeOfDay": [
        "late_night"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "youth",
        "challenge"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "深夜2時半の軽快なテンションと遊び心をテーマにした作品です。",
      "recommendedFor": "モヤモヤした夜を吹き飛ばしたい方や、ライブのような勢いのある音楽で気分を盛り上げたい方に適しています。",
      "recommendedSituation": "深夜のドライブ中や、眠れない夜にヘッドホンでボリュームを上げて楽しみたい時に合います。",
      "notRecommendedSituation": "一日の疲れを癒やす穏やかな就寝前の時間や、静かで集中力を要する作業中のBGMには賑やかすぎるため向いていません。",
      "recommendationText": "深夜2時半の軽快なテンションと、遊び心あるロックンロールの世界を描いた作品です。深夜のちょっとしたドライブの時間や、気分を切り替えたい夜に合います。",
      "listeningSuggestion": "ドライブや夜の部屋のスピーカーで、軽快なテンポに合わせて聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "timeOfDay",
        "condition": "late_night",
        "text": "真夜中にテンションを上げたい時、歪んだギターサウンドを思わせる響きと歌声が適度なスパイスになり得ます。"
      },
      {
        "conditionType": "situation",
        "condition": "driving",
        "text": "夜風を浴びながら疾走するドライブシーンで、曲が持つエネルギッシュなサウンドが爽快な疾走感に調和します。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、深夜の熱量をポップかつロックに描くエネルギッシュな楽曲。夜更けのライブ感あふれる勢いを重視し、emotionalIntensityを4、軽快で楽しげな性質からnostalgiaを2に設定。既存の穏やかなスコア（energy:3, liveHype:3）との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 80
    },
    "releases": [
      {
        "title": "「天使は何処へ」",
        "releaseType": "single",
        "releaseDate": "2023-04-12",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/26/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/26/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    id: 'me-konomama-de-momentary',
    title: 'このままでモーメンタリ',
    group: 'not-equal-me',
    category: 'unknown',
    factualData: {
      releaseDate: '2023-04-12',
      releaseTitle: '「天使は何処へ」',
      centerMembers: ["蟹沢萌子"],
      singingMembers: [],
      lyricist: '指原莉乃',
      composer: 'ヤナガワタカオ',
      arranger: 'ヤナガワタカオ',
      verificationStatus: 'verified',
      verifiedSources: [
        { field: 'release', url: 'https://not-equal-me.jp/discography/detail/23/' }
      ]
    },
    scores: {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    tags: {
      "moods": [
            "romantic",
            "dreamy"
      ],
      "situations": [
            "relaxing",
            "alone_time"
      ],
      "weather": [],
      "timeOfDay": [
            "evening",
            "night"
      ],
      "seasons": [
            "spring"
      ],
      "themes": [
            "love",
            "youth"
      ],
      "tempos": [
            "medium"
      ],
      "playlistRoles": [
            "cool_down"
      ]
    },
    recommendation: {
      songImpression: '儚く美しい「今この瞬間」の愛おしさを、幻想的で透明感のあるサウンドに乗せて歌うミディアムナンバーと受け取れます。',
      recommendedFor: '大切な人との時間の儚さに寄り添いたい方や、ドリーミーで心地よい浮遊感のあるポップスを求める方に適しています。',
      recommendedSituation: '夕暮れ時に窓の外を眺めながら過ごす時間や、夜に部屋を少し暗くしてリラックスするひとときに向いています。',
      notRecommendedSituation: '早朝の爽やかな目覚めと共にテンションを急上昇させたい時や、ハードな運動で汗を流したい場面には不向きです。',
      recommendationText: 'かけがえのない一瞬のきらめきと、胸に秘めた切ない青春の感情を、爽快で心地よいビートに乗せて歌うポップスです。',
      listeningSuggestion: '夕暮れ時の帰り道や、爽やかな風を感じながらノスタルジックな青春の思い出に浸りたい時におすすめです。',
      otherDisplayText: '一瞬の尊いきらめきを描く、爽やかで切ない青春ソング。'
    },
    recommendationVariants: [
      {
            "conditionType": "timeOfDay",
            "condition": "evening",
            "text": "夕空の色が変わる瞬間に聴くことで、曲が描く「瞬間」の美しさと切なさがより胸に深く響くでしょう。"
      },
      {
            "conditionType": "situation",
            "condition": "relaxing",
            "text": "肩の力を抜いて静かに過ごしたい時間に流すことで、ドリーミーな旋律が心に優しく寄り添います。"
      }
],
    enabled: true,
    recordType: 'song',
    needsReview: true,
    reviewRequirement: 'needs_review'
  },
  {
    id: 'me-ultra-rare-kiss',
    title: 'ウルトラレアキッス',
    group: 'not-equal-me',
    category: 'unknown',
    factualData: {
      releaseDate: '2022-11-23',
      releaseTitle: '「はにかみショート」',
      centerMembers: [],
      singingMembers: ["尾木波菜","菅波美玲","本田珠由記"],
      lyricist: '指原莉乃',
      composer: 'Yu-ki Kokubo, YUU for YOU',
      arranger: 'YUU for YOU',
      verificationStatus: 'verified',
      verifiedSources: [
        { field: 'release', url: 'https://not-equal-me.jp/discography/detail/22/' }
      ]
    },
    scores: {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    tags: {
      "moods": [
            "happy",
            "cute"
      ],
      "situations": [
            "getting_ready",
            "walking"
      ],
      "weather": [],
      "timeOfDay": [
            "morning",
            "daytime"
      ],
      "seasons": [
            "winter"
      ],
      "themes": [
            "love",
            "youth"
      ],
      "tempos": [
            "upbeat"
      ],
      "playlistRoles": [
            "build_up"
      ]
    },
    recommendation: {
      songImpression: 'コミカルでハイテンションな可愛らしさを、弾けるような超高速のビートとユニークな歌詞表現で描いたポップチューンと受け取れます。',
      recommendedFor: 'とことん明るくコミカルなアイドルソングで笑顔になりたい方や、とにかく元気に気分を盛り上げたい方に適しています。',
      recommendedSituation: '朝の目覚まし時間や、気合を入れて出かける準備をしたい時、テンションを上げたい通勤・通学路に向いています。',
      notRecommendedSituation: '静かな深夜にリラックスして読書をしたい時や、クラシック音楽を聴くような静粛を重んじる場面には賑やかすぎるため不向きです。',
      recommendationText: 'ゲームのようなポップな世界観を取り入れた、コミカルかつ賑やかで最高にキュートな自己紹介風ソングです。',
      listeningSuggestion: 'テンションを上げて楽しくなりたい時や、カラフルでコミカルな世界に浸りたい時におすすめです。',
      otherDisplayText: 'ゲームの世界観を描く、コミカルで賑やかなポップス。'
    },
    recommendationVariants: [
      {
            "conditionType": "mood",
            "condition": "cute",
            "text": "甘くキュートな遊び心に満ちた世界観を体感したい瞬間に聴くことで、曲の弾けるエネルギーがハッピーな気分を後押しします。"
      },
      {
            "conditionType": "situation",
            "condition": "getting_ready",
            "text": "朝の身支度を整える時間に流すことで、曲が持つハイテンションなサウンドが一日のスタートを軽快に彩るでしょう。"
      }
],
    enabled: true,
    recordType: 'song',
    needsReview: true,
    reviewRequirement: 'needs_review'
  },
  {
    "id": "me-gekkabijin",
    "title": "月下美人",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2023-09-06",
      "releaseTitle": "「想わせぶりっこ」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/38/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "bittersweet",
        "healing"
      ],
      "situations": [
        "relaxing",
        "before_sleep"
      ],
      "weather": [
        "cloudy",
        "snowy"
      ],
      "timeOfDay": [
        "late_night"
      ],
      "seasons": [
        "winter"
      ],
      "themes": [
        "love",
        "unrequited_love"
      ],
      "tempos": [
        "slow"
      ],
      "playlistRoles": [
        "ending"
      ]
    },
    "recommendation": {
      "songImpression": "夜の静寂に咲く月下美人のように、儚くも情熱的な秘めた恋心を和のエッセンスを感じる美しい調べで歌い上げた情緒溢れるミディアムバラードです。",
      "recommendedFor": "叙情的な歌詞世界に深く沈み込みたい方や、夜の静けさの中で切ない情念に耳を傾けたい方に強くおすすめします。",
      "recommendedSituation": "深夜の一人きりのリラックスタイムや、静かな部屋でキャンドルを灯して心を落ち着かせたい時に最適です。",
      "notRecommendedSituation": "朝の爽やかなスタートダッシュや、屋外での激しい運動中のBGMとしてはテンポや世界観が合わず適していません。",
      "recommendationText": "艶やかで切ない歌声と繊細なストリングスが重なり合い、胸の奥に眠る寂しさと美しい情愛を優しく呼び覚まします。",
      "listeningSuggestion": "楽器の息遣いと細やかなボーカル表現の機微を余すところなく捉えるため、静寂な夜にスピーカーから穏やかに流すのがおすすめです。"
    },
    "recommendationVariants": [
      {
        "conditionType": "timeOfDay",
        "condition": "late_night",
        "text": "深い夜の静けさの中で、内に秘めた切ない想いや美しい情景にじっくりと向き合う時間に寄り添います。"
      },
      {
        "conditionType": "situation",
        "condition": "relaxing",
        "text": "一日の終わりに肩の力を抜き、切なくも美しい和の旋律に心身を委ねて癒やされるひとときを演出します。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、夜の静寂に映える儚くも情熱的な和風ミディアムバラード。秘めた想いの回想から nostalgia は4、サビで高まる情念の深さから emotionalIntensity は4と評価。既存の落ち着いたステータス（calmness:3, sadness:3）との大きな矛盾は見られない。既存の季節タグは維持するが、既存タグや季節タグの整合性の確認は人間レビューが必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「想わせぶりっこ」",
        "releaseType": "single",
        "releaseDate": "2023-09-06",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/38/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/38/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    id: 'me-marshmallow-float',
    title: 'マシュマロフロート',
    group: 'not-equal-me',
    category: 'unknown',
    factualData: {
      releaseDate: '2023-04-12',
      releaseTitle: '「天使は何処へ」',
      centerMembers: ["菅波美玲"],
      singingMembers: [],
      lyricist: '指原莉乃',
      composer: 'BE YOU, 田辺望, 永野小織',
      arranger: '石黒剛',
      verificationStatus: 'verified',
      verifiedSources: [
        { field: 'release', url: 'https://not-equal-me.jp/discography/detail/24/' }
      ]
    },
    scores: {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 3
    },
    tags: {
      "moods": [
            "happy",
            "cute"
      ],
      "situations": [
            "getting_ready",
            "walking"
      ],
      "weather": [],
      "timeOfDay": [
            "morning",
            "daytime"
      ],
      "seasons": [
            "spring"
      ],
      "themes": [
            "love",
            "friendship"
      ],
      "tempos": [
            "medium"
      ],
      "playlistRoles": [
            "build_up"
      ]
    },
    recommendation: {
      songImpression: 'フワフワとした甘い恋心と日常の喜びを描いた、軽快でキャッチーなメロディが特徴的なキュートポップスと受け取れます。',
      recommendedFor: '甘酸っぱい恋愛ソングを聴いて明るい気分になりたい方や、日常のスタートに元気をもらいたい方に適しています。',
      recommendedSituation: '朝の身支度を整える時間や、天気の良い休日の朝にお散歩をしながら聴くのに向いています。',
      notRecommendedSituation: '深い感傷に浸りたい一人きりの静かな夜や、重厚なロックサウンドを求めて集中したい時には世界観が合わず不向きです。',
      recommendationText: 'マシュマロのようにふわふわした甘い恋心と、大好きな人に夢中な女の子の揺れる気持ちを可愛らしく表現した一曲です。',
      listeningSuggestion: 'カフェでのんびりお茶をする時間や、甘くハッピーな片思いのワクワク感に浸りたい時にぴったりです。',
      otherDisplayText: 'ふわふわした甘い片思いを可愛らしく描くポップソング。'
    },
    recommendationVariants: [
      {
            "conditionType": "mood",
            "condition": "cute",
            "text": "甘くハッピーな気分に浸りたい瞬間、弾けるようなボーカルがその可愛らしい感情に寄り添います。"
      },
      {
            "conditionType": "situation",
            "condition": "getting_ready",
            "text": "お出かけ前のメイク時間や服を選ぶ時間に聴くことで、一日を明るい気分でスタートする準備が整うでしょう。"
      }
],
    enabled: true,
    recordType: 'song',
    needsReview: true,
    reviewRequirement: 'needs_review'
  },
  {
    id: 'me-floor-killer',
    title: 'フロアキラー',
    group: 'not-equal-me',
    category: 'unknown',
    factualData: {
      releaseDate: '2023-09-06',
      releaseTitle: '「想わせぶりっこ」',
      centerMembers: ["落合希来里"],
      singingMembers: [],
      lyricist: '指原莉乃',
      composer: '近藤圭一',
      arranger: '近藤圭一, Uio',
      verificationStatus: 'verified',
      verifiedSources: [
        { field: 'release', url: 'https://not-equal-me.jp/discography/detail/37/' }
      ]
    },
    scores: {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 5
    },
    tags: {
      "moods": [
            "excited",
            "confident"
      ],
      "situations": [
            "getting_ready",
            "driving"
      ],
      "weather": [],
      "timeOfDay": [
            "night",
            "late_night"
      ],
      "seasons": [
            "summer"
      ],
      "themes": [
            "challenge",
            "identity"
      ],
      "tempos": [
            "upbeat"
      ],
      "playlistRoles": [
            "peak"
      ]
    },
    recommendation: {
      songImpression: 'ダンスビートを感じさせるサウンドと活動的な表現が融合した、高揚感を演出するアッパーチューンと解釈されます。',
      recommendedFor: 'エネルギッシュなダンス調の音楽で気分を高めたい方や、ライブハウスのような臨場感を好む方に適しています。',
      recommendedSituation: '気持ちを盛り上げて出かける準備をする時や、夜のドライブ中にビートを感じて走り抜ける時間に向いています。',
      notRecommendedSituation: '就寝前のリラックスした読書時間や、静かにクラシック音楽を聴くような落ち着いた空間には適していません。',
      recommendationText: '躍動感あふれるダンスと、夜のフロアを熱く盛り上げるエネルギッシュでクールなサウンドが魅力のダンスナンバーです。',
      listeningSuggestion: '気分を上げてアクティブに動き出したい時や、パワフルなダンスビートで盛り上がりたい時におすすめです。',
      otherDisplayText: '夜のフロアを熱く揺らす、エネルギッシュなダンス曲。'
    },
    recommendationVariants: [
      {
            "conditionType": "mood",
            "condition": "excited",
            "text": "エネルギーが高まっている時、この重厚なダンスサウンドがさらに心拍数を刺激するような高揚感をもたらします。"
      },
      {
            "conditionType": "situation",
            "condition": "driving",
            "text": "夜間のドライブシーンにおいて、ビートに同期するヘッドライトの光を感じながら走るような高揚感を生み出し得ます。"
      }
],
    enabled: true,
    recordType: 'song',
    needsReview: true,
    reviewRequirement: 'needs_review'
  },
  {
    "id": "me-kochira-happy-tankentai",
    "title": "こちらハッピー探検隊",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2022-08-03",
      "releaseTitle": "「す、好きじゃない！」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/16/"
        }
      ],
      "centerMembers": [],
      "lyricist": "指原莉乃",
      "composer": "小野寺祐輔",
      "arranger": "脇眞富"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "playful"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "friendship",
        "youth"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "明るく親しみやすい冒険心を歌う、元気でポップなメロディが特徴的なアッパーチューンと受け取れます。",
      "recommendedFor": "みんなで明るい気分を共有したい方や、日常の中の小さな探検気分を味わいたい方に適しています。",
      "recommendedSituation": "朝の身支度を整える時間や、お天気の良い日の気軽なお出かけ時のBGMに向いています。",
      "notRecommendedSituation": "厳かで深い考え事をしたい深夜の時間や、哀愁漂うセンチメンタルな余韻に浸りたい時には適していません。",
      "recommendationText": "河口夏音がセンターを務める5人組ユニットが、日常のハッピーを探して冒険するコミカルで楽しい探検隊ソングです。",
      "listeningSuggestion": "賑やかで楽しいハッピーポップで明るい気持ちになりたい時間に合います。",
      "otherDisplayText": "日常のハッピーを探して冒険するコミカルで楽しい探検隊ソング"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "playful",
        "text": "何か新しい面白いことを見つけたいようなワクワクした気分の時に、この陽気なメロディがその気持ちを応援します。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "お出かけの準備をする時間に流すことで、軽快なサウンドがスムーズなスタートを後押ししてくれるでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、探検と友情をテーマにしたポップソング。明るい前向きさを表現するためemotionalIntensityを4、爽快な日常感からnostalgiaを2に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「す、好きじゃない！」",
        "releaseType": "single",
        "releaseDate": "2022-08-03",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/16/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/16/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    id: 'me-summer-chocolate',
    title: 'サマーチョコレート',
    group: 'not-equal-me',
    category: 'unknown',
    factualData: {
      releaseDate: '2022-08-03',
      releaseTitle: '「す、好きじゃない！」',
      centerMembers: [],
      singingMembers: ["鈴木瞳美","谷崎早耶"],
      lyricist: '指原莉乃',
      composer: '加藤弘也',
      arranger: '杉原亮, 早川博隆',
      verificationStatus: 'verified',
      verifiedSources: [
        { field: 'release', url: 'https://not-equal-me.jp/discography/detail/15/' }
      ]
    },
    scores: {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 3
    },
    tags: {
      "moods": [
            "happy",
            "cute"
      ],
      "situations": [
            "getting_ready",
            "walking"
      ],
      "weather": [],
      "timeOfDay": [
            "morning",
            "daytime"
      ],
      "seasons": [
            "summer"
      ],
      "themes": [
            "love",
            "youth"
      ],
      "tempos": [
            "medium"
      ],
      "playlistRoles": [
            "build_up"
      ]
    },
    recommendation: {
      songImpression: '夏の甘くキュートな恋模様を描いた、フワフワと可愛らしいメロディが特徴的な甘口ポップスと受け取れます。',
      recommendedFor: '甘い恋愛ソングを聴いてハッピーな気分になりたい方や、とにかく可愛らしい音楽で癒やされたい方に適しています。',
      recommendedSituation: '夏の朝の身支度を整える時間や、天気の良い休日の散歩をしながら軽やかに聴くのに向いています。',
      notRecommendedSituation: '厳かでシリアスな集中力を要する作業時間や、重厚なロックサウンドを求めてテンションを高めたい場面には不向きです。',
      recommendationText: '夏のおしゃれや甘いチョコレートのような恋心を、王道の甘く可愛らしいメロディで弾けるように表現した楽曲です。',
      listeningSuggestion: '夏のレジャーへ向かうドライブ中や、とびきりキュートな気分でテンションを上げたい時に最適です。',
      otherDisplayText: '夏のおしゃれと甘い恋心を歌う、ラブリーなポップソング。'
    },
    recommendationVariants: [
      {
            "conditionType": "mood",
            "condition": "cute",
            "text": "とことん甘くハッピーな気分に浸りたい瞬間、弾けるようなボーカルがその可愛らしい感情にそっと寄り添います。"
      },
      {
            "conditionType": "situation",
            "condition": "getting_ready",
            "text": "お出かけ前のメイク時間や服を選ぶ時間に聴くことで、一日を明るい気分でスタートする準備が整うでしょう。"
      }
],
    enabled: true,
    recordType: 'song',
    needsReview: true,
    reviewRequirement: 'needs_review'
  },
  {
    id: 'me-momoiro-daybreak',
    title: '桃色デイブレイク',
    group: 'not-equal-me',
    category: 'unknown',
    factualData: {
      releaseDate: '2022-11-23',
      releaseTitle: '「はにかみショート」',
      centerMembers: ["櫻井もも"],
      singingMembers: [],
      lyricist: '指原莉乃',
      composer: '磯崎健史',
      arranger: '磯崎健史',
      verificationStatus: 'verified',
      verifiedSources: [
        { field: 'release', url: 'https://not-equal-me.jp/discography/detail/19/' }
      ]
    },
    scores: {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    tags: {
      "moods": [
            "excited",
            "dramatic"
      ],
      "situations": [
            "getting_ready",
            "commute"
      ],
      "weather": [],
      "timeOfDay": [
            "morning",
            "daytime"
      ],
      "seasons": [
            "all_seasons"
      ],
      "themes": [
            "youth",
            "identity"
      ],
      "tempos": [
            "upbeat"
      ],
      "playlistRoles": [
            "build_up"
      ]
    },
    recommendation: {
      songImpression: '新しい朝の訪れと内なる決意を感じさせる、疾走感のある劇的でエネルギッシュなメロディが特徴的な楽曲と受け取れます。',
      recommendedFor: '強い決意を持って新しい一日をスタートしたい方や、勢いのあるドラマチックなロック調のサウンドを好む方に適しています。',
      recommendedSituation: '夜明けから朝にかけての目覚めの時間や、気持ちを引き締めて移動する通学・通勤路に向いています。',
      notRecommendedSituation: '一日の疲れを穏やかに癒やしたい就寝前のベッドルームや、静かに読書を楽しみたい場面のBGMには賑やかすぎるため不向きです。',
      recommendationText: '圧倒的な歌唱力とシリアスな表情で、夜明け前の強い決意を力強くエッジの効いたサウンドで歌い上げるロックナンバーです。',
      listeningSuggestion: '何か大きな挑戦を控えていて闘志を燃やしたい時や、力強いボーカルで鼓舞されたい時に最適です。',
      otherDisplayText: '夜明け前の強い決意を歌う、エッジの効いたロック曲。'
    },
    recommendationVariants: [
      {
            "conditionType": "timeOfDay",
            "condition": "morning",
            "text": "朝の澄んだ空気の中で聴くことで、曲が描く夜明けの決意と爽快な熱量がより鮮明に心に響くでしょう。"
      },
      {
            "conditionType": "situation",
            "condition": "commute",
            "text": "移動時間や通学路で流すことで、ドラマチックなメロディラインが一日のスタートを力強く後押しします。"
      }
],
    enabled: true,
    recordType: 'song',
    needsReview: true,
    reviewRequirement: 'needs_review'
  },
  {
    "id": "me-daremo-inai-mori-no-oku-de",
    "title": "誰もいない森の奥で⼀本の⽊が倒れたら⾳はするか？",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-11-10",
      "releaseTitle": "「まほろばアスタリスク」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/10/"
        }
      ],
      "centerMembers": [],
      "lyricist": "指原莉乃",
      "composer": "千葉“naotyu-”直樹",
      "arranger": "千葉“naotyu-”直樹"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "calm",
        "dreamy"
      ],
      "situations": [
        "relaxing",
        "alone_time"
      ],
      "weather": [],
      "timeOfDay": [
        "night",
        "late_night"
      ],
      "seasons": [
        "autumn"
      ],
      "themes": [
        "self_acceptance",
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "cool_down"
      ]
    },
    "recommendation": {
      "songImpression": "誰にも気づかれないような内面の葛藤や存在の問いかけを、穏やかで幻想的なアコースティック調のサウンドに乗せて優しく歌う楽曲と受け取れます。",
      "recommendedFor": "静かに自己の心と向き合いたい方や、孤独に優しく寄り添うような穏やかな世界観に浸りたい方に適しています。",
      "recommendedSituation": "深夜に一人きりの部屋で考え事をしながら過ごす時間や、静かに読書をする時のBGMに向いています。",
      "notRecommendedSituation": "テンションを上げて大勢で盛り上がりたいライブ前の瞬間や、激しいダンスビートを求めて活動したい場面には向きません。",
      "recommendationText": "映画の挿入歌にも起用されたユニット曲。孤独や自己の存在についてアコースティックなサウンドで静かに問いかける作品です。",
      "listeningSuggestion": "一人で静かに過ごす夜の時間に合います。",
      "otherDisplayText": "孤独や自己の存在について静かに問いかけるアコースティックな曲"
    },
    "recommendationVariants": [
      {
        "conditionType": "timeOfDay",
        "condition": "late_night",
        "text": "真夜中の静寂の中で聴くことで、曲が持つ問いかけるような優しさと幻想的な調べがより静かに胸に染み入るでしょう。"
      },
      {
        "conditionType": "situation",
        "condition": "alone_time",
        "text": "誰にも邪魔されない一人の時間に流すことで、静かなアコースティックサウンドが心の中に穏やかな安らぎをもたらします。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、内省的な問いかけを描いた幻想的なポップソング。穏やかな温もりを表現するためemotionalIntensityを4、静かな内省とノスタルジーからnostalgiaを4に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「まほろばアスタリスク」",
        "releaseType": "single",
        "releaseDate": "2021-11-10",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/10/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/10/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-sukida",
    "title": "好きだ！！！",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-11-10",
      "releaseTitle": "「まほろばアスタリスク」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/8/"
        }
      ],
      "centerMembers": [],
      "lyricist": "指原莉乃",
      "composer": "Keisuke Koyama",
      "arranger": "家原正樹"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "excited",
        "confident"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "溢れる想いと情熱をストレートに歌い上げる、疾走感に満ちたメロディが胸に響く全力のポップナンバーと受け取れます。",
      "recommendedFor": "まっすぐな恋心の強さにエネルギーをもらいたい方や、スピード感のあるキャッチーなメロディで気分を最高潮に高めたい方に適しています。",
      "recommendedSituation": "朝の身支度を整えながら気持ちを奮い立たせたい時や、お出かけ時の移動路でテンションを高めたい時に向いています。",
      "notRecommendedSituation": "一日の終わりに静かに落ち着いて眠りにつきたい夜や、ゆったりとしたクラシックを聴いて心を無にしたい場面には賑やかすぎるため不向きです。",
      "recommendationText": "谷崎早耶と冨田菜々風のダブルセンターによる、ストレートな愛を歌う明るくエネルギッシュな王道青春ソングです。",
      "listeningSuggestion": "気持ちをリフレッシュして前を向きたい時間に合います。",
      "otherDisplayText": "ストレートな愛を全力で届ける明るくエネルギッシュな青春ソング"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "excited",
        "text": "エネルギーが有り余っている時、この疾走感のあるサウンドがさらに気分を高揚させ、元気を後押しします。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "出かける準備の仕上げの時間に聴くことで、曲が持つ圧倒的な熱量がスムーズな一日のスタートを促すでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、ストレートな愛を叫ぶ情熱的なポップソング。圧倒的なエネルギーを表現するためemotionalIntensityを5、現在進行形の力強さからnostalgiaを2に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「まほろばアスタリスク」",
        "releaseType": "single",
        "releaseDate": "2021-11-10",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/8/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/8/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-bokutachi-no-image",
    "title": "僕たちのイマージュ",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2022-08-03",
      "releaseTitle": "「す、好きじゃない！」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/14/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "healing",
        "excited"
      ],
      "situations": [
        "walking",
        "alone_time"
      ],
      "weather": [
        "sunny",
        "rainy"
      ],
      "timeOfDay": [
        "evening"
      ],
      "seasons": [
        "autumn"
      ],
      "themes": [
        "youth",
        "dream"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "青春のまぶしさと未来への希望を爽快なアンサンブルで描き出し、どこか甘酸っぱいノスタルジーと晴れやかな余韻を残すナンバーです。",
      "recommendedFor": "青春の温かい絆や仲間との約束に思いを馳せたい方、心地よいメロディで胸を熱くしたい方に最適です。",
      "recommendedSituation": "夕暮れ時の下校時や、一日の終わりに静かに散歩をしながら物思いにふけりたい時に寄り添ってくれます。",
      "notRecommendedSituation": "激しいダンスビートでハイテンションに騒ぎたいパーティーシーンなどには雰囲気が合わず不向きです。",
      "recommendationText": "爽やかな風が吹き抜けるような透明感のあるメロディと未来を見据える歌詞の調和が、一歩前に進む勇気を優しく引き出してくれます。",
      "listeningSuggestion": "歌詞の美しい余韻とボーカルの清涼感あるハモりをじっくり味わうため、静かな部屋やイヤホンでのリスニングが適しています。"
    },
    "recommendationVariants": [
      {
        "conditionType": "situation",
        "condition": "walking",
        "text": "夕方の帰り道など、心地よいリズムに乗せて一歩ずつ前へ歩き出す時間にそっと並走してくれる楽曲です。"
      },
      {
        "conditionType": "weather",
        "condition": "rainy",
        "text": "雨の日に窓の外を眺めながら、大切な約束や未来の自分たちに静かに思いを巡らせる時間にマッチします。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、青春の絆と未来への希望を描く爽やかなミディアムナンバー。仲間との軌跡を回想するニュアンスから nostalgia は4、未来への前向きな想いの高まりから emotionalIntensity は4と判定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、既存タグや季節タグの整合性の確認は人間レビューが必要。",
      "confidence": 80
    },
    "releases": [
      {
        "title": "「す、好きじゃない！」",
        "releaseType": "single",
        "releaseDate": "2022-08-03",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/14/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/14/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-off-shoulder-knit",
    "title": "#おふしょるにっと",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2022-02-16",
      "releaseTitle": "「チョコレートメランコリー」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/13/"
        }
      ],
      "centerMembers": [],
      "lyricist": "指原莉乃",
      "composer": "yuma",
      "arranger": "yuma"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 3
    },
    "tags": {
      "moods": [
        "happy",
        "cute"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "winter"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "冬のお出かけ前の女の子のウキウキしたときめきを、ポップで愛らしいメロディに乗せて甘く歌うキュートな楽曲と受け取れます。",
      "recommendedFor": "お出かけ前にテンションを高めたい方や、甘酸っぱい恋愛ソングでハッピーな気分になりたい方に適しています。",
      "recommendedSituation": "冬の寒い朝の身支度の時間や、楽しみにしているデートやお出かけへの移動中におけるリスニングに向いています。",
      "notRecommendedSituation": "シリアスな気持ちで課題や仕事に集中したい時や、静かで重厚な音楽を聴いて心を落ち着かせたい夜には不向きです。",
      "recommendationText": "鈴木瞳美と谷崎早耶のダブルセンターでおくる、冬のデート前におふしょるにっとを着た女の子の「かわいい」ときめきを描いたユニット曲です。",
      "listeningSuggestion": "デート前のおめかしする特別な時間に合います。",
      "otherDisplayText": "デート前のおふしょるにっとを着た女の子のかわいいときめきを描く曲"
    },
    "recommendationVariants": [
      {
        "conditionType": "season",
        "condition": "winter",
        "text": "冬の冷たい空気の中で聴くことで、曲が描くニットの温もりと恋する乙女の可愛らしさがよりいっそう魅力的に感じられるでしょう。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "お出かけ前にメイクやヘアスタイルを整えながら聴くことで、ハッピーな気分をさらに高めてくれるでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、冬の日常と恋模様を可愛らしく描いたキュートなポップソング。愛らしいときめきを表現するためemotionalIntensityを3、冬の日常感からnostalgiaを3に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「チョコレートメランコリー」",
        "releaseType": "single",
        "releaseDate": "2022-02-16",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/13/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/13/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-kimi-wa-sparkle",
    "title": "君はスパークル",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2022-02-16",
      "releaseTitle": "「チョコレートメランコリー」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/11/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 4,
      "brightness": 4,
      "sadness": 2,
      "cuteness": 4,
      "power": 4,
      "calmness": 2,
      "romance": 5,
      "liveHype": 4,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "romantic",
        "excited"
      ],
      "situations": [
        "walking",
        "relaxing",
        "before_live"
      ],
      "weather": [
        "sunny",
        "cloudy"
      ],
      "timeOfDay": [
        "evening",
        "night"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "love",
        "youth",
        "longing"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "手の届かない輝く存在への憧れと、胸が高鳴る切ない恋心を、きらびやかで疾走感のあるサウンドに乗せた至高のアイドルポップ。",
      "recommendedFor": "片想いの切なさを抱えつつも、相手の輝きを尊く思う純粋な恋心に没入したい人に。",
      "recommendedSituation": "星空が広がる夜の帰り道や、憧れの人を想い浮かべながら夕暮れを歩く時間に。",
      "notRecommendedSituation": "静かで淡々としたビートを聴き、心を完全に落ち着かせて深い眠りにつきたい夜。",
      "recommendationText": "星屑のようなきらめきをまとった疾走感あふれるメロディが、切ない恋愛感情と完璧にシンクロする超名曲です。エモーショナルに高まるサビのボーカルが、胸の奥にある切ない高揚感を一気に引き出します。",
      "listeningSuggestion": "イントロからサビにかけての華やかな音の広がりを感じながら、音の波に身を任せて聴いてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "romantic",
        "text": "恋焦がれる胸の切なさと高揚感を最高潮に引き出します。きらめくメロディが、あなたの中の純粋な想いを鮮やかに彩ってくれます。"
      },
      {
        "conditionType": "timeOfDay",
        "condition": "night",
        "text": "静かな夜の街灯の下、きらきらと輝く憧れの人を想いながら聴くのに最適。エモーショナルなサウンドが夜の空気に溶け込みます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "releases": [
      {
        "title": "「チョコレートメランコリー」",
        "releaseType": "single",
        "releaseDate": "2022-02-16",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/11/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/11/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "憧れの存在への眩しく切ない想いを描いた、王道にして極めてきらびやかなポップソング。恋心の美しさを表現するためにromanceを最高の5に、切なさと憧憬を込めてnostalgiaを4、感情の奔流を反映してemotionalIntensityを4としました。特定の季節を決定づける明確な記述はないため、all_seasonsを設定し、根拠は人間レビューでの再確認を仰ぎます。",
      "confidence": 94
    }
  },
  {
    "id": "me-watashi-accent",
    "title": "ワタシアクセント",
    "group": "not-equal-me",
    "category": "group",
    "factualData": {
      "releaseDate": "2021-11-10",
      "releaseTitle": "「まほろばアスタリスク」",
      "verificationStatus": "verified",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/9/",
          "note": "Official Reaudit Verified Evidence Source"
        }
      ],
      "centerMembers": [
        "永田詩央里"
      ],
      "participatingMembers": [],
      "lyricist": "指原莉乃",
      "composer": "YAS",
      "arranger": "YAS"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "excited",
        "confident"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "night"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "challenge",
        "identity"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "自分の個性を強調するプライドと、クールで挑戦的な決意を歌った作品です。",
      "recommendedFor": "ブレない強さを持って自分を表現したい時や、エッジの効いたクールな楽曲でエネルギーを高めたい方に適しています。",
      "recommendedSituation": "勝負事や大切なイベントの前の準備時間、夜のクールな移動路でのリスニングに向いています。",
      "notRecommendedSituation": "一日の疲れを癒やす穏やかな就寝前の時間や、静かで集中力を要する読書中のBGMには刺激が強すぎるため適していません。",
      "recommendationText": "自分の個性を強調するプライドと、クールで挑戦的な決意を描いた作品です。自分自身の意志を強く確かめたいときや、自信を持って一歩を踏み出したい瞬間に合います。",
      "listeningSuggestion": "移動中やお出かけ前の準備時間に、自分の決意を呼び起こすように聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "confident",
        "text": "ブレない強さを持って自己を肯定したい時、この力強く重厚なサウンドがさらに自信を後押しします。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "大切な本番や勝負事の前に聴くことで、モチベーションを高め、気持ちをしっかりと引き締める準備が整うでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、強い決意と自己主張をクールに歌うダンスナンバー。圧倒的な熱量を表現するためemotionalIntensityを5、現在進行形のクールな主張からnostalgiaを2に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 84
    },
    "releases": [
      {
        "title": "「まほろばアスタリスク」",
        "releaseType": "single",
        "releaseDate": "2021-11-10",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/9/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/9/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-chozetsu-heroine",
    "title": "超絶ヒロイン",
    "group": "not-equal-me",
    "category": "group",
    "factualData": {
      "releaseDate": "2021-07-14",
      "releaseTitle": "「君はこの夏、恋をする」",
      "verificationStatus": "verified",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/7/",
          "note": "Official Reaudit Verified Evidence Source"
        }
      ],
      "centerMembers": [
        "谷崎早耶"
      ],
      "participatingMembers": [],
      "lyricist": "指原莉乃",
      "composer": "塚田耕平",
      "arranger": "古川貴浩"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "cute"
      ],
      "situations": [
        "getting_ready",
        "walking"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "love",
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "憧れの人へのまっすぐな片思いと、特別な存在になりたい気持ちを歌った作品です。",
      "recommendedFor": "ときめく王道恋愛ソングで明るい気分になりたい方や、純粋な憧れや恋心のきらめきを好む方に適しています。",
      "recommendedSituation": "よく晴れた休日の朝の散歩や、お出かけに向けてメイクや服を選ぶ楽しい身支度の時間に向いています。",
      "notRecommendedSituation": "深い失恋の哀愁に一人でじっくり浸りたい静かな夜や、落ち着いた雰囲気を最優先したい読書時間には華やかすぎるため適していません。",
      "recommendationText": "憧れの人へのまっすぐな片思いと、大切な人にとっての特別な存在になりたい気持ちを描いた作品です。誰かを一途に想う時間や、甘酸っぱい恋心を振り返りたい時間に合います。",
      "listeningSuggestion": "晴れた日の青空の下を歩きながら、まっすぐな恋心に想いを馳せて聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "cute",
        "text": "とことんハッピーできらきらした恋心に包まれたい瞬間、愛らしいボーカルがその甘い感情に寄り添います。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "お出かけ前のメイク時間や服を選ぶ時間に聴くことで、一日を最高に楽しい気分でスタートする準備が整うでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、憧れの存在を歌うきらびやかなアイドルポップス。眩しい恋心を表現するためemotionalIntensityを4、回想的な憧憬からnostalgiaを4に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「君はこの夏、恋をする」",
        "releaseType": "single",
        "releaseDate": "2021-07-14",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/7/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/7/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-overture",
    "title": "Overture",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-04-07",
      "releaseTitle": "「超特急 ≠ME行き」 ＜CD Only/ノイミー盤＞",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/4/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/2/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/3/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3
    },
    "tags": {
      "moods": [],
      "situations": [],
      "weather": [],
      "timeOfDay": [],
      "seasons": [],
      "themes": [],
      "tempos": [],
      "playlistRoles": []
    },
    "recommendation": {
      "songImpression": "楽曲情報を確認中です",
      "recommendedFor": "楽曲情報を確認中です",
      "recommendedSituation": "楽曲情報を確認中です",
      "notRecommendedSituation": "楽曲情報を確認中です",
      "recommendationText": "楽曲情報を確認中です",
      "listeningSuggestion": "楽曲情報を確認中です"
    },
    "recommendationVariants": [],
    "enabled": false,
    "enabledForRecommendation": false,
    "needsReview": true,
    "releases": [
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/ノイミー盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/4/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD＋DVD/初回限定盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/2/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/通常盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/3/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/4/",
        "https://not-equal-me.jp/discography/detail/2/",
        "https://not-equal-me.jp/discography/detail/3/"
      ],
      "checkedAt": "2026-07-01T02:50:43.721Z"
    },
    "reason": "overture",
    "recordType": "overture"
  },
  {
    "id": "me-kimi-to-boku-no-uta",
    "title": "君と僕の歌",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-04-07",
      "releaseTitle": "「超特急 ≠ME行き」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/2/"
        }
      ],
      "centerMembers": [],
      "lyricist": "指原莉乃",
      "composer": "バグベア",
      "arranger": "バグベア"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "motivated"
      ],
      "situations": [
        "getting_ready",
        "commute"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "spring"
      ],
      "themes": [
        "gratitude",
        "youth"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "ファンとメンバーの強い絆や感謝の気持ちを、爽やかで温かみのあるメロディに乗せて前向きに届けるポップスと受け取れます。",
      "recommendedFor": "未来への希望を感じてモチベーションを高めたい方や、心温まる前向きな絆を歌った曲を求める方に適しています。",
      "recommendedSituation": "新しい一日の始まりである朝の通学・通勤時間や、気持ちをすっきりと前向きに整えたい時に向いています。",
      "notRecommendedSituation": "静かな深夜にリラックスして思考を休めたい時や、切ない哀愁に深く浸りたい場面には明るすぎるため不向きです。",
      "recommendationText": "冨田菜々風がセンターを務め、ファンとメンバーの約束や強い絆、さらに大きいステージへ一緒に進む決意を歌うメッセージソングです。",
      "listeningSuggestion": "これまでの歩みを振り返り、未来を夢見る時間に合います。",
      "otherDisplayText": "ファンと一緒にさらに大きいステージへ向かう約束と絆を歌う曲"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "motivated",
        "text": "前を向いて新しいことに挑戦したい時、この温かくも前向きなサウンドが背中を優しく後押しします。"
      },
      {
        "conditionType": "situation",
        "condition": "commute",
        "text": "通学や通勤などの移動時間に流すことで、曲が描く爽快な青春の雰囲気が日常の道のりを明るく彩るでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、ファンへの感謝と未来への歩みを歌う爽快なポップソング。前向きな一体感を表現するためemotionalIntensityを4、青春の回想感と絆からnostalgiaを4に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/ノイミー盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/4/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD＋DVD/初回限定盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/2/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/通常盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 3,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/3/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/4/",
        "https://not-equal-me.jp/discography/detail/2/",
        "https://not-equal-me.jp/discography/detail/3/"
      ],
      "checkedAt": "2026-07-01T02:50:43.722Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-kuhaku-no-hana",
    "title": "空白の花",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-04-07",
      "releaseTitle": "「超特急 ≠ME行き」 ＜CD Only/ノイミー盤＞",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/4/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/2/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/3/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 5,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "sad",
        "healing"
      ],
      "situations": [
        "relaxing",
        "before_sleep"
      ],
      "weather": [
        "rainy",
        "cloudy"
      ],
      "timeOfDay": [
        "late_night",
        "evening"
      ],
      "seasons": [
        "spring"
      ],
      "themes": [
        "unrequited_love",
        "love"
      ],
      "tempos": [
        "slow"
      ],
      "playlistRoles": [
        "ending"
      ]
    },
    "recommendation": {
      "songImpression": "ピアノ一台と張り詰めた歌声のみで孤独と喪失の痛みを極限まで表現した、静寂と激情が胸を締め付ける珠玉のソロバラードです。",
      "recommendedFor": "悲しみに深く寄り添ってほしい夜や、心に空いた空白をありのままに受け止めたいと感じている方に強くお勧めします。",
      "recommendedSituation": "雨の日の静かな夕暮れ時や、深夜に部屋の明かりを落とし、一人きりでそっと寂しさに浸りたい時に深く響きます。",
      "notRecommendedSituation": "多人数で賑やかに楽しむアウトドアイベントや、賑やかな商業空間のBGMなど、外向的で明るい空間には全く合いません。",
      "recommendationText": "余計な装飾を削ぎ落としたピアノアレンジだからこそ、ボーカルの吐息一つから伝わる深い切なさが、聴く者の心を揺さぶります。",
      "listeningSuggestion": "張り詰めた空気感と微細な表現のグラデーションを逃さず聴き取るため、ノイズキャンセリングヘッドホンを用いて静寂の中で聴いてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "sad",
        "text": "胸が締め付けられるような深い喪失感を、静かな旋律と搾り出すような歌声でありのまま包み込んでくれる、悲しみに寄り添う曲です。"
      },
      {
        "conditionType": "weather",
        "condition": "rainy",
        "text": "窓の外に降る雨の音をバックに、心の中の余白に静かに目を向けながら、大切な記憶に想いを馳せる時間に調和します。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、ピアノと歌声だけで孤独を歌い上げる圧倒的なバラード。過去の大切な存在への深い追慕から nostalgia は5、胸に迫る感情の剥き出しの強さから emotionalIntensity は5と評価。既存の静かな属性との大きな矛盾は見られない。既存の季節タグは維持するが、既存タグや季節タグの整合性の確認は人間レビューが必要。",
      "confidence": 85
    },
    "releases": [
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/ノイミー盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 4,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/4/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD＋DVD/初回限定盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 4,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/2/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/通常盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 4,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/3/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/4/",
        "https://not-equal-me.jp/discography/detail/2/",
        "https://not-equal-me.jp/discography/detail/3/"
      ],
      "checkedAt": "2026-07-01T02:50:43.722Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-p-i-c",
    "title": "P.I.C.",
    "group": "not-equal-me",
    "category": "group",
    "factualData": {
      "releaseDate": "2021-04-07",
      "releaseTitle": "「超特急 ≠ME行き」",
      "verificationStatus": "verified",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/4/",
          "note": "Official Reaudit Verified Evidence Source"
        }
      ],
      "centerMembers": [
        "冨田菜々風"
      ],
      "participatingMembers": [],
      "lyricist": "指原莉乃",
      "composer": "NA.ZU.NA / Yu-ki Kokubo / YHANAEL",
      "arranger": "NA.ZU.NA"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "excited",
        "confident"
      ],
      "situations": [
        "getting_ready",
        "driving"
      ],
      "weather": [],
      "timeOfDay": [
        "night",
        "late_night"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "challenge",
        "identity"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "新しい自分への挑戦と、ダークでクールな世界観を描いた作品です。",
      "recommendedFor": "エッジの効いたクールなサウンドで気分を最大限に高めたい方や、力強いモチベーションを引き出したい方に適しています。",
      "recommendedSituation": "勝負事や大切なイベントの前の準備時間や、夜間のドライブ中にスピード感を感じたい時に向いています。",
      "notRecommendedSituation": "就寝前の穏やかに心を休めたい時間や、のんびりとアコースティックな音楽に癒やされたい場面には不向きです。",
      "recommendationText": "新しい自分への挑戦と、ダークでクールなダンス世界を描いた作品です。新しい一歩を踏み出す決意を確かめたいときや、目標に向かって進む瞬間に合います。",
      "listeningSuggestion": "何かに挑戦する前の時間に、自分を鼓舞するように聴いてみてください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "excited",
        "text": "エネルギーが高まっている時、この重厚なダンスサウンドがさらに心拍数を刺激するような高揚感をもたらします。"
      },
      {
        "conditionType": "situation",
        "condition": "driving",
        "text": "夜間のドライブシーンにおいて、ビートに同期するヘッドライトの光を感じながら走るような高揚感を生み出し得ます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、強い決意と不敵な挑戦をクールに歌うダンスナンバー。圧倒的な熱量を表現するためemotionalIntensityを5、現在進行形のクールな主張からnostalgiaを2に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 84
    },
    "releases": [
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/ノイミー盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 5,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/4/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD＋DVD/初回限定盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 5,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/2/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/通常盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 5,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/3/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/4/",
        "https://not-equal-me.jp/discography/detail/2/",
        "https://not-equal-me.jp/discography/detail/3/"
      ],
      "checkedAt": "2026-07-01T02:50:43.722Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-teyuuka-mirutentte-nani",
    "title": "てゆーか、みるてんって何？",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-04-07",
      "releaseTitle": "「超特急 ≠ME行き」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/4/"
        }
      ],
      "centerMembers": [],
      "lyricist": "指原莉乃",
      "composer": "yuma",
      "arranger": "yuma"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "playful"
      ],
      "situations": [
        "getting_ready",
        "walking"
      ],
      "weather": [],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "youth",
        "identity"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "コミカルでハイテンションな自己紹介調の楽しさを、キャッチーでコミカルなビートに乗せて歌うハッピーなポップスと受け取れます。",
      "recommendedFor": "とにかく明るく楽しい気分になりたい方や、コミカルで愛らしいアイドルポップスで笑顔になりたい方に適しています。",
      "recommendedSituation": "朝の目覚めの時間や、元気をもらってお出かけの準備をしたい時、何気ない日常の散歩に向いています。",
      "notRecommendedSituation": "厳かで静粛な雰囲気を求めるシリアスな作業時間や、一人で深く考え事をしたい静かな夜には賑やかすぎるため不向きです。",
      "recommendationText": "本田珠由記（みるてん）のユニークな魅力をユーモラスに描いた、コミカルで賑やかな自己紹介ソングです。",
      "listeningSuggestion": "明るく賑やかなサウンドで楽しく過ごしたい時間に合います。",
      "otherDisplayText": "本田珠由記の個性をユーモラスに描くコミカルな自己紹介ソング"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "playful",
        "text": "とことんコミカルで楽しい気分に浸りたい瞬間、ユニークなボーカルがそのハッピーな感情にそっと寄り添います。"
      },
      {
        "conditionType": "situation",
        "condition": "getting_ready",
        "text": "お出かけ前のメイクや服を選ぶ時間に聴くことで、一日を楽しい気分でスタートする準備が整うでしょう。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、ユニークでコミカルな可愛らしさを歌うハッピーなポップソング。溢れる楽しさを表現するためemotionalIntensityを4、現在進行形のハイテンションな性質からnostalgiaを2に設定。既存の穏やかなスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、根拠は人間レビューで確認が必要。",
      "confidence": 82
    },
    "releases": [
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/ノイミー盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 6,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/4/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/4/"
      ],
      "checkedAt": "2026-07-01T02:50:43.722Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-himitsu-incident",
    "title": "秘密インシデント",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-04-07",
      "releaseTitle": "「超特急 ≠ME行き」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/2/"
        }
      ],
      "centerMembers": [],
      "lyricist": "指原莉乃",
      "composer": "宮川麿",
      "arranger": "宮川麿"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "excited",
        "romantic",
        "happy"
      ],
      "situations": [
        "getting_ready",
        "commute",
        "walking"
      ],
      "weather": [
        "sunny"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "spring",
        "summer"
      ],
      "themes": [
        "love",
        "youth",
        "promise"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "opening",
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "恋の始まりを予感させる高揚感と、瑞々しいメロディラインが印象的なポップチューンです。",
      "recommendedFor": "胸が高鳴るような恋心を味わいたい時や、前向きな気持ちで一日を始めたい方にぴったりです。",
      "recommendedSituation": "朝の通勤通学路で、軽快な足取りとともに爽やかな空気の中で聴くのに適しています。",
      "notRecommendedSituation": "静かに一日の反省を行いたい夜や、集中して作業に取り組みたい時には少し賑やかすぎるかもしれません。",
      "recommendationText": "虹がかかる美しい空のもとで突然始まる秘密の恋を描く、爽やかで王道な青春ポップナンバーです。",
      "listeningSuggestion": "青空が広がる晴れた日の散歩時間に合います。",
      "otherDisplayText": "虹がかかる空のもとで、突然始まる秘密の恋を描く爽やかな青春ソング"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "excited",
        "text": "心躍るような恋のワクワク感をさらに高めたい時に最適です。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "releases": [
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/ノイミー盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 7,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/4/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD＋DVD/初回限定盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 7,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/2/"
      },
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/通常盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 7,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/3/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/4/",
        "https://not-equal-me.jp/discography/detail/2/",
        "https://not-equal-me.jp/discography/detail/3/"
      ],
      "checkedAt": "2026-07-01T02:50:43.722Z"
    },
    "recordType": "song",
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "青春の甘酸っぱさと恋愛のときめきをストレートに表現した構成となっています。",
      "confidence": 5
    }
  },
  {
    "id": "me-jibun-sanka",
    "title": "自分賛歌",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-04-07",
      "releaseTitle": "「超特急 ≠ME行き」 ＜CD＋DVD/初回限定盤＞",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/2/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 4,
      "brightness": 4,
      "sadness": 1,
      "cuteness": 4,
      "power": 4,
      "calmness": 2,
      "romance": 1,
      "liveHype": 4,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "happy",
        "motivated"
      ],
      "situations": [
        "commute",
        "study",
        "walking"
      ],
      "weather": [
        "sunny",
        "cloudy"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "all_seasons"
      ],
      "themes": [
        "self_acceptance",
        "identity",
        "challenge"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "change"
      ]
    },
    "recommendation": {
      "songImpression": "悩んだり立ち止まったりする日常を全肯定し、自分のペースで一歩ずつ進む勇気をくれるハートフルな応援ソング。",
      "recommendedFor": "自分自身に自信を持てず背中を押してほしい人や、日々の小さな努力を肯定して元気になりたい人に。",
      "recommendedSituation": "通学・通勤の途中や、新しい挑戦を始める前にそっと勇気をもらいたい昼下がりに。",
      "notRecommendedSituation": "ダークで重厚なビートに浸り、孤独な夜の切なさを噛み締めたいとき。",
      "recommendationText": "等身大の葛藤と自己肯定のメッセージが優しく胸に響く、最高に温かいポップナンバーです。親しみやすいメロディラインと包容力のある歌声が、あなたの強張った肩の力を抜き、前を向く元気をもたらします。",
      "listeningSuggestion": "歌詞の一言一言に耳を傾けながら、心の中で自分に拍手を送るように聴くのがおすすめです。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "support",
        "text": "日々の葛藤をありのままに肯定し、自分らしく歩み出す力を与えてくれます。優しさに包まれてモチベーションを高めたい時に最適です。"
      },
      {
        "conditionType": "situation",
        "condition": "commute",
        "text": "朝の通勤・通学路で聴くことで、今日も自分のペースで頑張ろうと心が軽くなります。前向きな一日を過ごすためのスタートダッシュにふさわしい楽曲です。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "reviewRequirement": "needs_review",
    "needsReview": true,
    "releases": [
      {
        "title": "「超特急 ≠ME行き」 ＜CD＋DVD/初回限定盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 6,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/2/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/2/"
      ],
      "checkedAt": "2026-07-01T02:50:43.722Z"
    },
    "recordType": "song",
    "analysisBasis": {
      "interpretationNotes": "自分自身の成長や歩んできた道を肯定する優しくも力強い応援ソング。明るさと励ましのニュアンスを重視し、brightnessとpowerを4に、nostalgiaは3、心の温かさを反映してemotionalIntensityを4に設定しました。特定の季節モチーフはなく、一年中寄り添うテーマのためall_seasonsを設定。根拠は人間レビューでの再確認を要します。",
      "confidence": 93
    }
  },
  {
    "id": "me-ponytail-kyurun",
    "title": "ポニーテール キュルン",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-04-07",
      "releaseTitle": "「超特急 ≠ME行き」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "release",
          "url": "https://not-equal-me.jp/discography/detail/3/"
        }
      ],
      "centerMembers": [],
      "lyricist": "指原莉乃",
      "composer": "野井洋児",
      "arranger": "家原正樹"
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 3
    },
    "tags": {
      "moods": [
        "cute",
        "playful",
        "happy"
      ],
      "situations": [
        "getting_ready",
        "alone_time",
        "celebration"
      ],
      "weather": [
        "sunny"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "summer"
      ],
      "themes": [
        "love",
        "youth",
        "jealousy"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "愛らしい歌声と華やかなリズムが融合した、非常にキュートでポップな楽曲です。",
      "recommendedFor": "可愛らしい雰囲いに浸りたい時や、自分磨きの時間をもっと楽しく彩りたい方に最適です。",
      "recommendedSituation": "休日の朝にお気に入りの服を選びながら、気分を盛り上げたい時によく馴染みます。",
      "notRecommendedSituation": "厳かな雰囲気の中で気持ちを落ち着かせたい時や、静かに読書を楽しみたい場面には向きません。",
      "recommendationText": "鈴木瞳美がセンターを務め、ポニーテールにした女の子の大好きな人への「かわいい」片思いを描くキュートなポップスです。",
      "listeningSuggestion": "甘酸っぱい胸のときめきを感じたい時間に合います。",
      "otherDisplayText": "ポニーテールをテーマに大好きな君への片思いを描くかわいらしい曲"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "cute",
        "text": "可愛らしさに包まれて、ハッピーな気分で満たされたい時におすすめです。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": false,
    "releases": [
      {
        "title": "「超特急 ≠ME行き」 ＜CD Only/通常盤＞",
        "releaseType": "album",
        "releaseDate": "2021-04-07",
        "trackNumber": 6,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/3/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/3/"
      ],
      "checkedAt": "2026-07-01T02:50:43.722Z"
    },
    "recordType": "song",
    "reviewRequirement": "human_approved",
    "analysisBasis": {
      "interpretationNotes": "夏の恋心とヘアスタイルへのこだわりをモチーフにした、愛らしさに特化した世界観です。",
      "confidence": 5
    }
  },
  {
    "id": "me-hakumeikousen",
    "title": "薄明光線",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2021-07-14",
      "releaseTitle": "「君はこの夏、恋をする」",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/6/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 5
    },
    "tags": {
      "moods": [
        "confident",
        "motivated"
      ],
      "situations": [
        "study",
        "driving"
      ],
      "weather": [
        "cloudy",
        "sunny"
      ],
      "timeOfDay": [
        "morning"
      ],
      "seasons": [
        "spring"
      ],
      "themes": [
        "challenge",
        "identity"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "build_up"
      ]
    },
    "recommendation": {
      "songImpression": "暗闇から光が差し込む瞬間を切り取ったかのような、ドラマチックなメロディラインと力強いボーカルが胸を突くエモーショナルな楽曲です。",
      "recommendedFor": "未来への希望を見出したい時、静かな決意と高揚感を胸に抱いて新たな一歩を踏み出したい方に適しています。",
      "recommendedSituation": "早朝の澄んだ空気の中で新しい一日を始める時や、何かに立ち向かうための心の準備を整えたい時に聴くのが最適です。",
      "notRecommendedSituation": "賑やかなパーティーや、頭を空っぽにして何も考えずにリラックスしたい場面には、サウンドの熱量が適さず不向きです。",
      "recommendationText": "雲の切れ間から差す光（薄明光線）を想起させるダイナミックな曲調が、聴き手の内に眠る勇気とモチベーションを呼び起こしてくれます。",
      "listeningSuggestion": "サビに向けて爆発するボーカルの圧倒的な歌唱力と、疾走するギターロックのダイナミズムを、音量を少し大きめにして体感してください。"
    },
    "recommendationVariants": [
      {
        "conditionType": "timeOfDay",
        "condition": "morning",
        "text": "少しずつ空が白んでいく夜明けの青い時間帯に聴くことで、曲が持つ光への決意が現実の景色と美しくシンクロします。"
      },
      {
        "conditionType": "mood",
        "condition": "motivated",
        "text": "困難を乗り越えて突き進もうとする強い意志を、芯のある歌声とドラマチックな旋律が力強く後押ししてくれます。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "曲の中心的な印象は、夜明けの希望と力強いボーカルが印象的なエモーショナルロック。劇的な高揚感と決意の強さから emotionalIntensity は5とし、未来志向のため nostalgia は3と評価。既存の力強いスコア構成との大きな矛盾は見られない。既存の季節タグは維持するが、既存タグや季節タグの整合性の確認は人間レビューが必要。",
      "confidence": 83
    },
    "releases": [
      {
        "title": "「君はこの夏、恋をする」",
        "releaseType": "single",
        "releaseDate": "2021-07-14",
        "trackNumber": 2,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/6/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/6/"
      ],
      "checkedAt": "2026-07-01T02:50:43.722Z"
    },
    "recordType": "song"
  },
  {
    "id": "me-guuzen-symphony",
    "title": "偶然シンフォニー",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2024-03-20",
      "releaseTitle": "「Springtime In You」ノイミー盤",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/49/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/46/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/48/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/47/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 3,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "excited",
        "dramatic",
        "happy"
      ],
      "situations": [
        "commute",
        "walking",
        "celebration"
      ],
      "weather": [
        "sunny"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "spring",
        "autumn"
      ],
      "themes": [
        "love",
        "youth",
        "friendship"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "偶然の出会いから広がるドラマチックな展開を、爽やかなメロディに乗せて描くアップテンポなナンバーです。",
      "recommendedFor": "青春のきらめきを感じたい時や、偶然の重なりに心躍らせたい時に適しています。",
      "recommendedSituation": "晴れた日の屋外イベントへの移動中など、期待感に胸を膨らしながら進みたい場面に向いています。",
      "notRecommendedSituation": "穏やかな波の音を聴きながらリラックスしたい夕暮れ時や、深い瞑想をしたい時には適しません。",
      "recommendationText": "躍動的なメロディと晴れやかなハーモニーが交差し、未来への明るい予感を抱かせます。",
      "listeningSuggestion": "少しテンポを意識して歩きながら、イヤホンで立体感のあるボーカルを楽しむのが良いでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "excited",
        "text": "胸の高鳴りをそのまま楽しさへと昇華させたい場面に向いています。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "releases": [
      {
        "title": "「Springtime In You」ノイミー盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/49/"
      },
      {
        "title": "「Springtime In You」初回限定豪華盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/46/"
      },
      {
        "title": "「Springtime In You」通常盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/48/"
      },
      {
        "title": "「Springtime In You」初回限定盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 1,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/47/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/49/",
        "https://not-equal-me.jp/discography/detail/46/",
        "https://not-equal-me.jp/discography/detail/48/",
        "https://not-equal-me.jp/discography/detail/47/"
      ],
      "checkedAt": "2026-07-01T02:52:42.149Z"
    },
    "recordType": "song",
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "偶然の連鎖から始まる奇跡をテーマに、希望と喜びを最大限に演出した楽曲です。",
      "confidence": 5
    }
  },
  {
    "id": "me-marcato",
    "title": "Marcato",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2024-03-20",
      "releaseTitle": "「Springtime In You」ノイミー盤",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/49/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/46/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/48/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/47/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "confident",
        "rebellious",
        "motivated"
      ],
      "situations": [
        "study",
        "work",
        "alone_time"
      ],
      "weather": [
        "cloudy",
        "clear_night"
      ],
      "timeOfDay": [
        "evening",
        "night"
      ],
      "seasons": [
        "winter",
        "all_seasons"
      ],
      "themes": [
        "independence",
        "challenge",
        "identity"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "change"
      ]
    },
    "recommendation": {
      "songImpression": "自立した強い意思とクールな世界観を、エッジの効いたメロディで表現したスタイリッシュな楽曲です。",
      "recommendedFor": "自分自身の信念を貫きたい時や、内に秘めた闘志を刺激したい方に向いています。",
      "recommendedSituation": "目標に向けて集中を高めたい作業前の時間や、ストイックに進みたいトレーニング時によく合います。",
      "notRecommendedSituation": "休日にひだまりの中でまどろみたい時や、甘く柔らかな恋愛気分を楽しみたい時には適しません。",
      "recommendationText": "重厚感のある響きとシャープなリズムが融和し、一歩を踏み出す勇気を奮い立たせます。",
      "listeningSuggestion": "低音の響きを重視したイヤホンで、力強いリズムラインに耳を傾けながら聴くのがおすすめです。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "confident",
        "text": "自分の意志を強く持ち、迷いなく前に進む覚悟を補強したい時に最適です。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "releases": [
      {
        "title": "「Springtime In You」ノイミー盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 6,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/49/"
      },
      {
        "title": "「Springtime In You」初回限定豪華盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 6,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/46/"
      },
      {
        "title": "「Springtime In You」通常盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 6,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/48/"
      },
      {
        "title": "「Springtime In You」初回限定盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 6,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/47/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/49/",
        "https://not-equal-me.jp/discography/detail/46/",
        "https://not-equal-me.jp/discography/detail/48/",
        "https://not-equal-me.jp/discography/detail/47/"
      ],
      "checkedAt": "2026-07-01T02:52:42.149Z"
    },
    "recordType": "song",
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "己の生き方を貫く決意を音楽記号に準えてスタイリッシュに描き出した構成です。",
      "confidence": 5
    }
  },
  {
    "id": "me-date-zenya-requiem",
    "title": "デート前夜レクイエム",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2024-03-20",
      "releaseTitle": "「Springtime In You」ノイミー盤",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/49/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/46/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/48/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/47/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 2,
      "emotionalIntensity": 4
    },
    "tags": {
      "moods": [
        "excited",
        "playful",
        "happy"
      ],
      "situations": [
        "getting_ready",
        "night_walk",
        "celebration"
      ],
      "weather": [
        "clear_night"
      ],
      "timeOfDay": [
        "night",
        "late_night"
      ],
      "seasons": [
        "autumn",
        "all_seasons"
      ],
      "themes": [
        "love",
        "youth",
        "longing"
      ],
      "tempos": [
        "upbeat"
      ],
      "playlistRoles": [
        "build_up",
        "peak"
      ]
    },
    "recommendation": {
      "songImpression": "特別な日の前夜における焦燥感と期待を、遊び心あふれる賑やかなテンポで歌うポップソングです。",
      "recommendedFor": "楽しみな予定を控えて落ち着かない夜や、夜更かしの時間をテンション高く過ごしたい方に適しています。",
      "recommendedSituation": "お出かけの準備を前夜から入念に行う時間や、明日への期待を胸に過ごしたい時に馴染みます。",
      "notRecommendedSituation": "翌日の早起きに備えて深い眠りにつきたい真夜中や、心を落ち着かせてリラクゼーションを図りたい時には適しません。",
      "recommendationText": "弾けるようなリズムと変化に富んだ構成が、明日の約束への待ち遠しさを鮮やかに彩ります。",
      "listeningSuggestion": "部屋を少し暗くして、ポップなメロディラインを小音量で流しながら楽しむのも面白いでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "excited",
        "text": "特別な約束を前にして、浮き足立つような興奮を分かち合いたい夜にぴったりです。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "releases": [
      {
        "title": "「Springtime In You」ノイミー盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 8,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/49/"
      },
      {
        "title": "「Springtime In You」初回限定豪華盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 8,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/46/"
      },
      {
        "title": "「Springtime In You」通常盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 8,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/48/"
      },
      {
        "title": "「Springtime In You」初回限定盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 8,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/47/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/49/",
        "https://not-equal-me.jp/discography/detail/46/",
        "https://not-equal-me.jp/discography/detail/48/",
        "https://not-equal-me.jp/discography/detail/47/"
      ],
      "checkedAt": "2026-07-01T02:52:42.149Z"
    },
    "recordType": "song",
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "大切な約束の前の心の揺らぎを、独創的なアレンジとコミカルなタッチで表現しています。",
      "confidence": 5
    }
  },
  {
    "id": "me-haru-no-koibito",
    "title": "春の恋人",
    "group": "not-equal-me",
    "category": "unknown",
    "factualData": {
      "releaseDate": "2024-03-20",
      "releaseTitle": "「Springtime In You」ノイミー盤",
      "verificationStatus": "needs-review",
      "verifiedSources": [
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/49/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/46/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/48/",
          "note": "Official Discography Page"
        },
        {
          "field": "title",
          "url": "https://not-equal-me.jp/discography/detail/47/",
          "note": "Official Discography Page"
        }
      ]
    },
    "scores": {
      "energy": 3,
      "brightness": 3,
      "sadness": 3,
      "cuteness": 3,
      "power": 3,
      "calmness": 3,
      "romance": 3,
      "liveHype": 3,
      "nostalgia": 4,
      "emotionalIntensity": 3
    },
    "tags": {
      "moods": [
        "calm",
        "healing",
        "bittersweet"
      ],
      "situations": [
        "relaxing",
        "walking",
        "alone_time"
      ],
      "weather": [
        "sunny",
        "breezy"
      ],
      "timeOfDay": [
        "morning",
        "daytime"
      ],
      "seasons": [
        "spring"
      ],
      "themes": [
        "love",
        "youth",
        "memories"
      ],
      "tempos": [
        "medium"
      ],
      "playlistRoles": [
        "change",
        "cool_down"
      ]
    },
    "recommendation": {
      "songImpression": "春の木漏れ日のような温かさと、どこか切ない余韻を秘めた優美なミディアムナンバーです。",
      "recommendedFor": "過ぎ去った青春の日々に思いを馳せたい時や、穏やかで優しい旋律に包まれたい方にぴったりです。",
      "recommendedSituation": "春風を感じる並木道を散歩する時間や、窓辺で温かい飲み物を片手にくつろぐ時に向いています。",
      "notRecommendedSituation": "大勢で盛り上がるフェスのような活気ある場面や、アップテンポな曲で汗を流したい時には不向きです。",
      "recommendationText": "柔らかなボーカルと美しいメロディが融和し、季節の移り変わりと揺れる恋心を優しく表現します。",
      "listeningSuggestion": "音量を少し控えめに設定し、背景の自然の音と重ね合わせながら聴くのが馴染みやすいでしょう。"
    },
    "recommendationVariants": [
      {
        "conditionType": "mood",
        "condition": "calm",
        "text": "静かに流れる時間に身を任せ、心地よいメランコリーに浸りたい時に適しています。"
      }
    ],
    "enabled": true,
    "enabledForRecommendation": true,
    "needsReview": true,
    "releases": [
      {
        "title": "「Springtime In You」ノイミー盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 10,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/49/"
      },
      {
        "title": "「Springtime In You」初回限定豪華盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 10,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/46/"
      },
      {
        "title": "「Springtime In You」通常盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 10,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/48/"
      },
      {
        "title": "「Springtime In You」初回限定盤",
        "releaseType": "album",
        "releaseDate": "2024-03-20",
        "trackNumber": 10,
        "officialSourceUrl": "https://not-equal-me.jp/discography/detail/47/"
      }
    ],
    "verification": {
      "titleVerified": true,
      "releaseVerified": true,
      "categoryVerified": false,
      "sourceContainsExactTitle": true,
      "verifiedSources": [
        "https://not-equal-me.jp/discography/detail/49/",
        "https://not-equal-me.jp/discography/detail/46/",
        "https://not-equal-me.jp/discography/detail/48/",
        "https://not-equal-me.jp/discography/detail/47/"
      ],
      "checkedAt": "2026-07-01T02:52:42.149Z"
    },
    "recordType": "song",
    "reviewRequirement": "needs_review",
    "analysisBasis": {
      "interpretationNotes": "春の訪れとともに揺れる恋心と別れの予感を、淡く美しい筆致で描き出しています。",
      "confidence": 5
    }
  }
];

// v7 Audit helpers
// id: 'love-24-7'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-onesan-ja-dame-desuka'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-weekend-citron'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-zuttomo-anken'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-shukusai'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-the-5th'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-ohimesama-ni-shiteyo'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-poison-girl'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-bpm170-no-kimi-e'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-egao-no-recipe'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-shirangedo'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-boku-no-heroine'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
// id: 'love-sukitte-ienakatta'
// description: ''
// recommendationText: ''
// listeningSuggestion: ''
// otherDisplayText: ''
//  {
